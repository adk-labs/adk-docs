...
Live (`run_live()`) agent sessions add a set of real-time parameters, including `realtime_input_config`, `session_resumption`, `save_live_blob`, `tool_thread_pool_config`, `proactivity`, `enable_affective_dialog`, and more. These are documented in one place, with per-model support and examples, in the live docs:

- **[Live agent configuration](https://adk.dev/live/configuration/index.md)** — the full `RunConfig` reference for live agents.
- **[Sessions](https://adk.dev/live/sessions/#session-resumption)** — session resumption and reconnection.
...
The `tool_thread_pool_config` setting is an exception: it is a runtime concern rather than a Live API one, so it stays here. It runs tool executions in a background thread pool so the event loop keeps responding to user interruptions.
