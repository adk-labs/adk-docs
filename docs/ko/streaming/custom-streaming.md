...
이 문서는 ADK 스트리밍과 [FastAPI](https://fastapi.tiangolo.com/)를 사용하여 구축된 커스텀 비동기 웹 앱의 서버 및 클라이언트 코드 개요를 다룹니다. 이 앱은 서버-전송 이벤트(SSE)를 통해 실시간 양방향 오디오 및 텍스트 통신을 가능하게 합니다. 주요 특징은 다음과 같습니다:

**서버 측 (Python/FastAPI)**:
- FastAPI + ADK 통합
- 실시간 스트리밍을 위한 서버-전송 이벤트(SSE)
- 격리된 사용자 컨텍스트를 통한 세션 관리
- 텍스트 및 오디오 통신 모드 모두 지원
- 근거 있는(grounded) 응답을 위한 Google 검색 도구 통합

**클라이언트 측 (JavaScript/Web Audio API)**:
- SSE 및 HTTP POST를 통한 실시간 양방향 통신
- AudioWorklet 프로세서를 사용한 전문적인 오디오 처리
- 텍스트와 오디오 모드 간의 원활한 전환
- 자동 재연결 및 오류 처리
- 오디오 데이터 전송을 위한 Base64 인코딩

이 샘플의 [WebSocket](custom-streaming-ws.md) 버전도 제공됩니다.
...
=== "Gemini - Google AI Studio"
    ...
    2. (`app/` 내에 위치한) **`.env`** 파일을 열고 다음 코드를 복사하여 붙여넣습니다.
    ...
    3. `PASTE_YOUR_ACTUAL_API_KEY_HERE`를 실제 `API 키`로 교체합니다.
...
=== "Gemini - Google Cloud Vertex AI"
    ...
    2. (`app/` 내에 위치한) **`.env`** 파일을 엽니다. 다음 코드를 복사하여 붙여넣고 프로젝트 ID와 위치를 업데이트합니다.
...
3. **텍스트 모드로 앱 접근:** 앱이 시작되면 터미널에 로컬 URL(예: [http://localhost:8000](http://localhost:8000))이 표시됩니다. 이 링크를 클릭하여 브라우저에서 UI를 엽니다.

이제 다음과 같은 UI가 표시됩니다:

![ADK 스트리밍 앱](../assets/adk-streaming-text.png)

`지금 몇 시야?`와 같은 질문을 해보세요. 에이전트는 Google 검색을 사용하여 쿼리에 응답합니다. UI는 에이전트의 응답을 스트리밍 텍스트로 표시하는 것을 볼 수 있습니다. 에이전트가 아직 응답하는 중에도 언제든지 메시지를 보낼 수 있습니다. 이는 ADK 스트리밍의 양방향 통신 기능을 보여줍니다.

4. **오디오 모드로 앱 접근:** 이제 `오디오 시작(Start Audio)` 버튼을 클릭합니다. 앱은 오디오 모드로 서버에 다시 연결되며, UI에는 처음으로 다음과 같은 대화 상자가 표시됩니다:
...
이제 음성으로 에이전트와 대화할 수 있습니다. `지금 몇 시야?`와 같은 질문을 음성으로 하면 에이전트가 음성으로 응답하는 것을 들을 수 있습니다. ADK용 스트리밍은 [여러 언어](https://ai.google.dev/gemini-api/docs/live#supported-languages)를 지원하므로, 지원되는 언어로 된 질문에도 응답할 수 있습니다.

5. **콘솔 로그 확인**

Chrome 브라우저를 사용하는 경우, 마우스 오른쪽 버튼을 클릭하고 `검사`를 선택하여 개발자 도구를 엽니다. `콘솔` 탭에서 `[CLIENT TO AGENT]` 및 `[AGENT TO CLIENT]`와 같은 들어오고 나가는 오디오 데이터를 볼 수 있으며, 이는 브라우저와 서버 간에 스트리밍되는 오디오 데이터를 나타냅니다.
...
- **브라우저가 SSH 프록시를 통해 서버에 연결할 수 없는 경우:** 다양한 클라우드 서비스에서 사용되는 SSH 프록시는 SSE와 작동하지 않을 수 있습니다. 로컬 노트북을 사용하는 등 SSH 프록시 없이 시도하거나, [WebSocket](custom-streaming-ws.md) 버전을 시도해 보세요.
- **`gemini-2.0-flash-exp` 모델이 작동하지 않는 경우:** 앱 서버 콘솔에서 `gemini-2.0-flash-exp` 모델 가용성과 관련된 오류가 표시되면, `app/google_search_agent/agent.py` 파일의 6번째 줄에서 `gemini-2.0-flash-live-001`로 교체해 보세요.
...
root_agent = Agent(
   ...
   model="gemini-2.0-flash-exp", # 이 모델이 작동하지 않으면 아래를 시도해 보세요
...
[Google 검색을 통한 근거 제시(grounding)](https://ai.google.dev/gemini-api/docs/grounding?lang=python#configure-search) 기능이 얼마나 쉽게 통합되었는지 주목하세요. `Agent` 클래스와 `google_search` 도구는 LLM 및 검색 API와의 복잡한 상호작용을 처리하므로, 개발자는 에이전트의 *목적*과 *행동*에 집중할 수 있습니다.
...
#### 클라이언트에서 에이전트로의 흐름:
1. **연결 수립** - 클라이언트가 `/events/{user_id}`로 SSE 연결을 열면 세션 생성이 트리거되고 요청 큐가 `active_sessions`에 저장됩니다.
2. **메시지 전송** - 클라이언트는 `mime_type`과 `data`를 포함하는 JSON 페이로드를 `/send/{user_id}`로 POST 전송합니다.
3. **큐 처리** - 서버는 세션의 `live_request_queue`를 검색하고 `send_content()` 또는 `send_realtime()`을 통해 에이전트로 메시지를 전달합니다.

#### 에이전트에서 클라이언트로의 흐름:
1. **이벤트 생성** - 에이전트는 요청을 처리하고 `live_events` 비동기 제너레이터를 통해 이벤트를 생성합니다.
2. **스트림 처리** - `agent_to_client_sse()`는 이벤트를 필터링하고 SSE 호환 JSON으로 포맷합니다.
3. **실시간 전달** - 이벤트는 적절한 SSE 헤더와 함께 영구적인 HTTP 연결을 통해 클라이언트로 스트리밍됩니다.

#### 세션 관리:
- **사용자별 격리** - 각 사용자는 `active_sessions` 딕셔너리에 저장된 고유한 세션을 갖습니다.
- **라이프사이클 관리** - 연결이 끊어지면 적절한 리소스 해제와 함께 세션이 자동으로 정리됩니다.
...
#### 세션 재개:
- **라이브 세션 재개** - 중단된 라이브 대화에 투명하게 재연결할 수 있습니다.
- **핸들 캐싱** - 시스템은 복구를 위해 세션 핸들을 자동으로 캐시합니다.
- **신뢰성 향상** - 스트리밍 중 네트워크 불안정성에 대한 복원력을 향상시킵니다.
...
- **InMemoryRunner 설정** - 앱 이름 "ADK 스트리밍 예제"와 Google 검색 에이전트를 사용하여 메모리에서 에이전트 라이프사이클을 관리하는 러너 인스턴스를 생성합니다.

- **세션 생성** - `runner.session_service.create_session()`을 사용하여 사용자 ID별로 고유한 세션을 설정하여 여러 동시 사용자를 지원합니다.

- **응답 양식 구성** - `is_audio` 매개변수에 따라 `RunConfig`를 "AUDIO" 또는 "TEXT" 양식으로 설정하여 출력 형식을 결정합니다.

- **LiveRequestQueue** - 들어오는 요청을 큐에 넣고 클라이언트와 에이전트 간의 실시간 메시지 전달을 가능하게 하는 양방향 통신 채널을 생성합니다.

- **라이브 이벤트 스트림** - `runner.run_live()`는 부분 응답, 턴 완료, 중단을 포함하여 에이전트로부터 실시간 이벤트를 생성하는 비동기 제너레이터를 반환합니다.
...
async def agent_to_client_sse(live_events):
    ...
    async for event in live_events:
        ...
        # 오디오인 경우 Base64 인코딩된 오디오 데이터 전송
...
- **이벤트 처리 루프** - `live_events` 비동기 제너레이터를 반복하며 에이전트에서 도착하는 각 이벤트를 처리합니다.

- **턴 관리** - 대화 턴 완료 또는 중단 이벤트를 감지하고 `turn_complete` 및 `interrupted` 플래그가 포함된 JSON 메시지를 전송하여 대화 상태 변경을 알립니다.

- **콘텐츠 파트 추출** - 텍스트 또는 오디오 데이터가 포함된 이벤트 콘텐츠에서 첫 번째 `Part`를 추출합니다.

- **오디오 스트리밍** - PCM 오디오 데이터를 다음과 같이 처리합니다:
  - `inline_data`에서 `audio/pcm` MIME 타입 감지
  - JSON 전송을 위해 원시 오디오 바이트를 Base64 인코딩
  - `mime_type` 및 `data` 필드와 함께 전송

- **텍스트 스트리밍** - 생성되는 대로 증분 텍스트 업데이트를 전송하여 실시간 타이핑 효과를 구현하는 부분 텍스트 응답을 처리합니다.
...
async def sse_endpoint(user_id: int, is_audio: str = "false"):
    ...
    print(f"클라이언트 #{user_id}가 SSE를 통해 연결됨, 오디오 모드: {is_audio}")

    def cleanup():
        live_request_queue.close()
        if user_id_str in active_sessions:
            del active_sessions[user_id_str]
        print(f"클라이언트 #{user_id}가 SSE에서 연결 해제됨")
...
**GET /events/{user_id}** - 영구적인 SSE 연결을 설정합니다:
...
- **정리 로직** - 요청 큐를 닫고 활성 세션에서 제거하여 연결 종료를 처리하며, 스트림 중단에 대한 오류 처리를 포함합니다.

### 세션 재개 구성

ADK는 스트리밍 대화 중 신뢰성을 향상시키기 위해 라이브 세션 재개를 지원합니다. 이 기능은 네트워크 문제로 인해 라이브 연결이 중단되었을 때 자동 재연결을 가능하게 합니다.

#### 세션 재개 활성화

세션 재개를 활성화하려면 다음을 수행해야 합니다:
...
- **자동 핸들 캐싱** - 시스템은 라이브 대화 중에 세션 재개 핸들을 자동으로 캐시합니다.
- **투명한 재연결** - 연결이 중단되면 시스템은 캐시된 핸들을 사용하여 재개를 시도합니다.
- **컨텍스트 보존** - 대화 컨텍스트와 상태는 재연결 시에도 유지됩니다.
- **네트워크 복원력** - 불안정한 네트워크 조건에서 더 나은 사용자 경험을 제공합니다.

#### 구현 참고 사항

- 세션 재개 핸들은 ADK 프레임워크에 의해 내부적으로 관리됩니다.
- 추가적인 클라이언트 측 코드 변경은 필요하지 않습니다.
- 이 기능은 장시간 실행되는 스트리밍 대화에 특히 유용합니다.
- 연결 중단이 사용자 경험에 미치는 영향이 줄어듭니다.

#### 문제 해결

세션 재개와 관련하여 오류가 발생하는 경우:

1. **모델 호환성 확인** - 세션 재개를 지원하는 모델을 사용하고 있는지 확인하세요.
2. **API 제한** - 일부 세션 재개 기능은 모든 API 버전에서 사용 가능하지 않을 수 있습니다.
3. **세션 재개 제거** - 문제가 지속되면 `RunConfig`에서 `session_resumption` 매개변수를 제거하여 세션 재개를 비활성화할 수 있습니다.
...
async def send_message_endpoint(user_id: int, request: Request):
    ...
    if mime_type == "text/plain":
        ...
        return {"error": f"지원되지 않는 Mime 타입: {mime_type}"}
...
- **메시지 처리** - `mime_type`과 `data` 필드가 있는 JSON을 파싱합니다:
  ...
  - **오디오 메시지** - PCM 데이터를 Base64 디코딩하고 `Blob`과 함께 `send_realtime()`을 통해 전송합니다.
...
- **텍스트 입력 양식** - 텍스트 메시지를 위한 입력 필드 및 전송 버튼
- **오디오 제어** - 오디오 모드 및 마이크 접근을 활성화하는 버튼

### 주요 애플리케이션 로직 (`static/js/app.js`)
...
- **임의 세션 ID** - 각 브라우저 인스턴스에 대한 고유 세션 ID 생성
- **URL 구성** - 세션 ID를 사용하여 SSE 및 전송 엔드포인트 구성
- **오디오 모드 플래그** - 오디오 모드 활성화 여부 추적
...
- **EventSource 설정** - 오디오 모드 매개변수와 함께 SSE 연결 생성
- **연결 핸들러**:
  - **onopen** - 연결 시 전송 버튼 및 양식 제출 활성화
  - **onmessage** - 에이전트로부터 들어오는 메시지 처리
  - **onerror** - 5초 후 자동 재연결로 연결 끊김 처리

#### 메시지 처리 (`app.js`)
서버로부터의 다양한 메시지 유형 처리:
...
- **오디오 재생** - Base64 PCM 데이터를 디코딩하여 오디오 워크릿으로 전송
- **텍스트 표시** - 새 메시지 요소를 생성하고 실시간 타이핑 효과를 위해 부분 텍스트 업데이트 추가
...
- **HTTP POST** - `/send/{session_id}` 엔드포인트로 JSON 페이로드 전송
- **오류 처리** - 실패한 요청 및 네트워크 오류 로깅
...
- **AudioContext 설정** - 재생을 위해 24kHz 샘플 레이트로 컨텍스트 생성
- **워크릿 로딩** - 오디오 처리를 위한 PCM 플레이어 프로세서 로드
- **오디오 파이프라인** - 워크릿 노드를 오디오 대상(스피커)에 연결

### 오디오 레코더 (`static/js/audio-recorder.js`)

**startAudioRecorderWorklet()** 함수:

- **AudioContext 설정** - 녹음을 위해 16kHz 샘플 레이트로 컨텍스트 생성
- **마이크 접근** - 오디오 입력을 위한 사용자 미디어 권한 요청
- **오디오 처리** - 마이크를 레코더 워크릿에 연결
...
- **링 버퍼** - 24kHz 오디오 180초 분량의 원형 버퍼
- **데이터 수집** - Int16을 Float32로 변환하여 버퍼에 저장
...
- **데이터 전송** - Float32 샘플을 복사하여 메시지 포트를 통해 메인 스레드로 게시

#### 모드 전환:
- **오디오 활성화** - "오디오 시작" 버튼이 마이크를 활성화하고 오디오 플래그와 함께 SSE를 다시 연결
- **원활한 전환** - 기존 연결을 닫고 새로운 오디오 지원 세션 설정

클라이언트 아키텍처는 텍스트와 오디오 양식 모두에서 원활한 실시간 통신을 가능하게 하며, 전문적인 수준의 오디오 처리를 위해 최신 웹 API를 사용합니다.
...
**아키텍처 하이라이트**:
- **실시간**: 부분 텍스트 업데이트와 연속 오디오를 통한 스트리밍 응답
...
#### 보안
- **인증**: 임의 세션 ID를 적절한 사용자 인증으로 교체
- **API 키 보안**: 환경 변수 또는 시크릿 관리 서비스 사용
- **HTTPS**: 모든 통신에 TLS 암호화 강제
- **속도 제한**: 남용 방지 및 API 비용 제어

#### 확장성
- **영구 스토리지**: 인메모리 세션을 영구 세션으로 교체
- **로드 밸런싱**: 공유 세션 상태로 여러 서버 인스턴스 지원
- **오디오 최적화**: 대역폭 사용량 감소를 위한 압축 구현

#### 모니터링
- **오류 추적**: 시스템 장애 모니터링 및 경고
- **API 비용 모니터링**: 예산 초과 방지를 위한 Google 검색 및 Gemini 사용량 추적
