...
Part 1에서는 ADK Gemini Live API Toolkit 애플리케이션의 4단계 라이프사이클을 배웠습니다.
...
**ADK Gemini Live API Toolkit에서 Content와 Part 사용하기:**
...
- **`Part`** (`google.genai.types.Part`): 메시지 내부의 개별 콘텐츠 단위입니다.
  ADK Gemini Live API Toolkit + Live API에서는 주로 다음을 사용합니다.
...
!!! note "ADK Gemini Live API Toolkit에서의 Content/Part 사용"
...
ADK Gemini Live API Toolkit에서는 애플리케이션이 `close` 신호를 명시적으로 보내야 합니다.
...
([`base_llm_flow.py:1150`](https://github.com/google/adk-python/blob/427a983b18088bdc22272d02714393b0a779ecdf/src/google/adk/flows/llm_flows/base_llm_flow.py#L1150)).
...
모범 사례를 학습했습니다.
이제 ADK Gemini Live API Toolkit 애플리케이션에서 업스트림 통신 채널로
