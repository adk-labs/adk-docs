...
- **Phase 1: 애플리케이션 초기화** (애플리케이션 시작 시 1회)
  - ADK 애플리케이션 초기화
    - 사용자와 상호작용하고, 외부 도구를 사용하고, 다른 에이전트와 협력할 [Agent](https://google.github.io/adk-docs/agents/) 생성
    - ADK `Session`을 조회하거나 생성하기 위한 [SessionService](https://google.github.io/adk-docs/sessions/session/#managing-sessions-with-a-sessionservice) 생성
    - 에이전트를 실행할 [Runner](https://google.github.io/adk-docs/runtime/) 생성
...
ADK [Session](https://google.github.io/adk-docs/sessions/session/)은 스트리밍 세션 전반의 대화 상태와 기록을 관리합니다. 세션 데이터를 저장하고 불러오며, 대화 재개와 컨텍스트 지속 같은 기능을 제공합니다.

`Session`을 생성하거나, 특정 `session_id`에 대한 기존 세션을 가져오려면, 모든 ADK 애플리케이션은 [SessionService](https://google.github.io/adk-docs/sessions/session/#managing-sessions-with-a-sessionservice)가 필요합니다. 개발용으로는 `InMemorySessionService`를 제공하며, 애플리케이션 종료 시 `Session` 상태가 사라집니다.
...
[Runner](https://google.github.io/adk-docs/runtime/)는 `Agent`의 런타임을 제공합니다. 대화 흐름을 관리하고, 도구 실행을 조율하며, 이벤트를 처리하고, 세션 저장소와 통합합니다. 애플리케이션 시작 시 한 번 생성해 모든 스트리밍 세션에서 재사용합니다.
...
- **[ADK (Agent Development Kit)](https://google.github.io/adk-docs/)**: 스트리밍 기능을 갖춘 AI 에이전트를 구축하는 Google의 프로덕션 프레임워크
