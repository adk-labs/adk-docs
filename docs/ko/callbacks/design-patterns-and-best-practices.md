...
**패턴 개요:**
관측 가능성(observability) 및 디버깅을 위해 특정 라이프사이클 지점에 상세한 로깅을 추가합니다.
...
**오류를 우아하게 처리:**
- 콜백 함수 내에서 `try...except/catch` 블록을 사용하세요.
- 오류를 적절히 로깅하십시오.
...
- 의도하지 않은 부작용을 피하기 위해 광범위한 구조를 수정하기보다는 특정 상태 키를 사용하세요.
- 특히 영구 `SessionService` 구현 시 명확성을 위해 상태 접두사(`State.APP_PREFIX`, `State.USER_PREFIX`, `State.TEMP_PREFIX`) 사용을 고려하세요.
...
- 통합 테스트를 수행하여 콜백이 전체 에이전트 흐름 내에서 올바르게 작동하는지 확인하세요.

**명확성 보장:**
- 콜백 함수에 서술적인 이름을 사용하세요.
- 함수의 목적, 실행 시점, 부작용(특히 상태 수정)을 설명하는 명확한 독스트링(docstring)을 추가하세요.
