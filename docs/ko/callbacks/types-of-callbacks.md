...
**목적:** 정리 작업, 실행 후 검증, 에이전트 활동 완료 로깅 또는 최종 상태 수정에 유용합니다.

!!! note "After Agent Callback 출력 수정 제한"

    `after_agent_callback`은 에이전트가 AI 모델을 여러 번 호출하고 여러 이벤트를 생략했을 수 있기 때문에 응답 출력을 완전히 변경할 수 없습니다. 따라서 출력을 수정하는 것은 허용되지 않지만, 추가 콘텐츠를 *덧붙일* 수는 있습니다. AI 모델 응답을 변경하려면 `after_model_callback`을 고려하세요.
...
*   **동작 방식:** 콜백 함수(`modify_output_after_agent`)는 세션 상태의 `add_concluding_note` 플래그를 확인합니다.
    *   플래그가 `True`이면 콜백은 *새로운* `types.Content` 객체를 반환합니다. 이는 ADK 프레임워크에게 에이전트의 원본 출력에 콜백이 반환한 콘텐츠를 **덧붙이도록** 지시합니다.
...
*   **예상 결과:** 두 가지 시나리오를 볼 수 있습니다:
    ...
    2. 해당 상태 플래그가 있는 세션에서는 콜백이 에이전트의 원본 출력을 가로채서 자신의 메시지("Concluding note added...")를 덧붙입니다.
