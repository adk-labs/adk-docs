...
프로덕션 호스팅 환경에 에이전트를 배포할 때는 제한된 도구와 서비스에 제대로 인증하는 능력이 더 중요하고 관리가 어려워집니다. 사용자의 권한이 제각각일 경우에는 이 문제가 더 복잡해집니다. 여러 도구의 인증 흐름과 자격 증명 관리를 직접 구현하는 대신, 둘 다 대신 처리해 주는 *인증 관리자* 서비스를 사용하세요. 이 서비스는 키와 시크릿의 저장은 물론 OAuth 액세스 토큰과 새로 고침 토큰의 획득, 관리, 저장까지 담당해야 합니다.
