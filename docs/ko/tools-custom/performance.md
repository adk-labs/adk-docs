...
그러나 이 성능 향상을 사용하려면 사용자 지정 도구를 비동기 실행 지원으로 빌드해야 합니다. 이 가이드에서는 ADK에서 병렬 도구 실행이 작동하는 방식과 이 처리 기능을 최대한 활용하도록 도구를 빌드하는 방법을 설명합니다.
...
도구 함수를 비동기 함수로 정의하여 병렬 실행을 활성화합니다. Python 코드에서 이는 ADK가 `asyncio` 이벤트 루프에서 동시에 실행할 수 있도록 하는 `async def` 및 `await` 구문을 사용하는 것을 의미합니다. 다음 섹션에서는 병렬 처리 및 비동기 작업을 위해 빌드된 에이전트 도구의 예를 보여줍니다.
...
사용자가 여러 정보를 요청할 때 항상 함수를 병렬로 호출하십시오.
  ...
  항상 단일 복잡한 호출보다 여러 특정 함수 호출을 선호하십시오.
```

다음 예는 병렬 실행을 통해 보다 효율적인 사용을 암시하는 도구 함수 설명을 보여줍니다.

```python
 async def get_weather(city: str) -> dict:
      """단일 도시에 대한 현재 날씨를 가져옵니다.
