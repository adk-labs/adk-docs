...
1. **[함수 도구](/ko/tools-custom/function-tools/):** 특정 애플리케이션의 요구에 맞게 직접 만든 도구입니다.
    * **[함수/메서드](/ko/tools-custom/function-tools/#1-function-tool):** 코드에서 표준 동기 함수 또는 메서드를 정의합니다(예: Python def).
    * **[도구로서의 에이전트](/ko/tools-custom/function-tools/#3-agent-as-a-tool):** 다른, 잠재적으로 특수화된 에이전트를 상위 에이전트의 도구로 사용합니다.
    * **[장기 실행 함수 도구](/ko/tools-custom/function-tools/#2-long-running-function-tool):** 비동기 작업을 수행하거나 완료하는 데 상당한 시간이 걸리는 도구를 지원합니다.
2. **[기본 제공 도구](/ko/tools/built-in-tools/):** 일반적인 작업을 위해 프레임워크에서 제공하는 즉시 사용 가능한 도구입니다.
...
인증 흐름, 구성 및 예제에 대한 자세한 설명은 전용 도구 인증 설명서 페이지를 참고하세요.
...
* **매개변수(인수):**
    ...
    * **모델이 제공해야 하는 정보에는 기본값을 피하세요.** 예를 들어 목적지가 사용자 또는 대화 컨텍스트에서 나와야 한다면 `def my_func(destination: str = "Paris")` 같은 형태는 피하세요. 기본값은 실제로 선택적인 튜닝 파라미터에는 적절할 수 있지만, 도구 스키마에서 필수 비즈니스 입력을 숨기는 용도로 사용하지 마세요.
...
* **반환 유형:**
    ...
    * 사전/맵 키와 값을 **LLM이 쉽게 이해할 수 있도록** 설명적으로 설계하십시오. 모델은 이 출력을 읽고 다음 단계를 결정한다는 점을 기억하세요.
...
* **단순성 및 집중:**
    ...
    * **복잡한 작업 분해:** 여러 고유한 논리적 단계를 수행하는 함수를 더 작고 집중된 도구로 분해합니다. 예를 들어 단일 `update_user_profile(profile: ProfileObject)` 도구 대신 `update_user_name(name: str)`, `update_user_address(address: str)`, `update_user_preferences(preferences: list[str])` 등과 같은 별도의 도구를 고려하세요. 이렇게 하면 LLM이 올바른 기능을 더 쉽게 선택하고 사용할 수 있습니다.
