...
이 가이드에서는 ADK 에이전트 워크플로를 재개 가능하도록 구성하는 방법을 설명합니다. 사용자 지정 에이전트를 사용하는 경우 재개 가능하도록 업데이트할 수 있습니다. 자세한 내용은 [사용자 지정 에이전트에 재개 추가](#custom-agents)를 참고하세요.
...
!!! info "참고: 사용자 지정 에이전트"
    ...
    [사용자 지정 에이전트에 재개 추가](#custom-agents)를 참고하세요.
...
-   **CustomAgentState 클래스 만들기**: BaseAgentState를 확장하여 에이전트의 상태를 보존하는 개체를 만듭니다.
    -   **선택적으로 WorkFlowStep 클래스 만들기**: 사용자 지정 에이전트에 순차적 단계가 있는 경우 에이전트의 개별적이고 저장 가능한 단계를 정의하는 WorkFlowStep 목록 개체를 만드는 것을 고려하세요.
