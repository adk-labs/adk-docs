...
에이전트 실행을 구성할 때 `RunConfig`를 전달하여 에이전트가 모델과 상호 작용하고, 오디오를 처리하며, 응답을 스트리밍하는 방식을 사용자 정의할 수 있습니다. 기본적으로 스트리밍은 활성화되어 있지 않으며 입력은 아티팩트로 보존되지 않습니다. `RunConfig`를 사용하여 이러한 기본값을 재정의하세요.
