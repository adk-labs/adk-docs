...
기존 평가 케이스는 그대로 유지할 수 있습니다. 동일한 대화 시나리오나 고정된 대화를 사용하여 텍스트 실행과 음성 실행 모두를 구동할 수 있으므로 기존 스위트를 재사용할 수 있습니다. 전체 스키마, 페르소나 및 시나리오 작성법은 [오디오 사용자 시뮬레이션](../evaluate/user-sim.md#audio-user-simulation-for-live-agents)을 참고하세요.
