# Java로 실시간 스트리밍 에이전트 구축하기

ADK Streaming을 사용하여 지연 시간이 짧은 양방향 음성 대화를 나누는 Java 에이전트를 구축합니다.

Java와 Maven을 설정하고, 프로젝트 종속성을 정의한 뒤 `ScienceTeacherAgent`를 구축합니다. 먼저 Dev UI에서 텍스트 스트리밍으로 테스트한 다음, 라이브 오디오를 활성화하여 대화해 봅니다.
...
*   Java용 **Maven** 빌드 도구가 필요하므로 진행하기 전에 [Maven을 설치](https://maven.apache.org/install.html)하세요(Cloud Top과 Cloud Shell에는 이미 설치되어 있지만, 로컬 개발 환경에는 없을 수 있습니다).
...
다음으로 음성 및 턴(turn) 감지를 설정하려면 [구성](../configuration.md)을, 라이브 에이전트에 도구를 제공하려면 [도구](../tools.md)를 참고하세요.
