...
??? note "ADK의 에이전트 카드에 대한 추가 정보"
    ...
    ADK에서는 `to_a2a(root_agent)` 래퍼를 사용하여 에이전트 카드를 자동으로 생성할 수 있습니다. 다른 사람이 사용할 수 있도록 기존 에이전트를 노출하는 방법에 대해 자세히 알아보려면 [A2A 빠른 시작(노출)](quickstart-exposing.md) 자습서를 참고하세요.
