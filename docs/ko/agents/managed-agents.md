...
- **에이전트 ID:** Gemini API와 마찬가지로 `agent_id`가 필요합니다. [에이전트 생성 및 관리 가이드](https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/managed-agents/create-manage)를 통해 생성하거나, 프로젝트에서 제공되는 기본 에이전트 ID를 사용합니다.
