...
Python과 Java 모두에서 ADK와 함께 Anthropic의 Claude 모델을 사용할 수 있습니다. 아래에서 사용하는 언어 및 백엔드에 맞는 경로를 선택하세요.
...
다음 코드 예시는 에이전트에서 Claude 모델을 사용하기 위한 기본 구현을 보여줍니다:
...
원하는 Claude 모델 이름과 API 키로 구성된 `AnthropicOkHttpClient`를 제공하여 `com.google.adk.models.Claude`를 인스턴스화합니다. 그런 다음 아래 예시와 같이 `Claude` 인스턴스를 `LlmAgent`에 전달합니다:

```java
import com.anthropic.client.AnthropicClient;
import com.google.adk.agents.LlmAgent;
import com.google.adk.models.Claude;
import com.anthropic.client.okhttp.AnthropicOkHttpClient; // Anthropic SDK

public class DirectAnthropicAgent {

  private static final String CLAUDE_MODEL_ID = "claude-sonnet-4-6"; // 또는 선호하는 Claude 모델

  public static LlmAgent createAgent() {

    // 보안 구성에서 중요한 키를 로드하는 것이 좋습니다
    AnthropicClient anthropicClient = AnthropicOkHttpClient.builder()
        .apiKey("ANTHROPIC_API_KEY")
        .build();
