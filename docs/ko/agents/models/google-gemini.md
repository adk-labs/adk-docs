...
[컨텍스트 캐싱](/ko/context/caching/),
[컴퓨터 사용](/ko/tools/gemini-api/computer-use/),
...
??? note "참고: Gemini 모델 셀렉터 `gemini-flash-latest`"

    ADK 문서의 대부분 예제는 최신 사용 가능한 Gemini Flash 버전을 선택하기 위해
    `gemini-flash-latest`를 사용합니다. 다만 Gemini를 지역 엔드포인트(예:
    `us-central1`)에서 사용하는 경우 이 문자열이 동작하지 않을 수 있습니다.
    그런 경우에는 [Gemini 모델](https://ai.google.dev/gemini-api/docs/models)
    페이지 또는 Google Cloud의 [Gemini 모델](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/models)
    목록에서 특정 모델 버전 문자열을 사용하세요.
...
2.  클라이언트 측 재시도를 활성화합니다. 재시도는 일시적인 할당량 문제 시 대기 후 자동으로 요청을 다시 보내어 완화할 수 있습니다.
    ...
    **옵션 2:** 모델 어댑터의 재시도 옵션을 설정합니다.
    ...
    이 옵션은 이 모델 어댑터 인스턴스를 직접 생성하는 경우에 적합합니다.
...
=== "Java"
    ...
    // 참고: Java ADK의 Interactions API 지원은 현재 개발 중입니다.
    LlmAgent rootAgent = LlmAgent.builder()
        .model(Gemini.builder()
            .modelName("gemini-flash-latest")
            .build())
        .name("interactions_test_agent")
        .tools(
            GoogleSearchTool.INSTANCE, // 검색 도구
            getCurrentWeather // 커스텀 함수 도구
        )
        .build();
    ```
...
Interactions API는 [Google Search](/ko/tools/built-in-tools/#google-search)와 같은
...
=== "Java"
    ...
    ```java
    // 참고: bypassMultiToolsLimit는 Python 전용입니다.
    // Java에서는 도구 인스턴스를 그대로 사용하세요.
