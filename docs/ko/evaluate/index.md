...
ADK는 실측 자료 기반 및 루브릭 기반 도구 사용 평가 메트릭을 모두 제공합니다. 에이전트의 특정 요구 사항 및 목표에 적합한 메트릭을 선택하려면 [기준 권장 사항](#recommendations-on-criteria)을 참고하세요.
...
### 일치성 테스트로 평가 (Conformance testing)

`adk conformance test` 명령어는 AI 에이전트가 시간이 지나도 일관되게 작동하는지 확인합니다. 이는 현재 에이전트의 출력을 기준 데이터(baseline)와 검증하여 코드베이스나 모델의 업데이트가 회귀(regression)를 유발하지 않도록 보장합니다.

#### 사전 요구 사항 및 설정

`adk conformance` 명령어가 의미 있는 회귀 테스트를 실행하려면 먼저 최적의 "골든 기준선(golden baseline)"을 설정해야 합니다. 일치성 테스트는 활성 에이전트의 동작을 이전에 기록되고 검증된 상호 작용과 비교하는 방식으로 작동합니다.

환경을 준비하려면 다음 워크플로를 따르세요.

##### 테스트 디렉터리 계층 구조 생성

일치성 테스트는 테스트 케이스를 자동으로 검색하고 매핑하기 위해 엄격한 파일 레이아웃에 의존합니다. 다음 구조를 사용하여 테스트 디렉터리를 초기화합니다.

```
tests
└── category_name/
    └── test_case_name/
        ├── spec.yaml                  # 테스트 케이스 사양
        ├── generated-recordings.yaml   # 기준선 기록된 상호 작용
        └── generated-session.yaml      # 기준선 세션 데이터
```

!!! note
    에이전트가 서버 전송 이벤트(SSE)를 사용하는 경우, 테스트 프레임워크는 동일한 폴더 내에서 추가로 `generated-recordings-sse.yaml` 및 `generated-session-sse.yaml`을 찾습니다.

##### 테스트 사양 정의 (spec.yaml)

대상 테스트 폴더에 `spec.yaml` 파일을 생성합니다. 이 파일은 기준선 기록 및 후속 일치성 실행 동안 에이전트가 실행할 초기 조건, 구성 및 사용자 프롬프트를 개략적으로 설명합니다. 파일이 다음 기본 스키마와 일치하는지 확인하세요 (예시일 뿐입니다).

```yaml
# Weather Agent에 대한 spec.yaml 예시
name: "current_weather_check"
description: "에이전트가 위치를 정확히 식별하고 날씨 도구를 호출하는지 검증합니다."
user_prompts:
  - "지금 샌프란시스코 기온이 어때?"
expected_tools:
  - "get_weather_api"
...
백그라운드 데이터(LLM 요청 및 도구 호출 등)가 복잡하기 때문에 기준선 파일을 수동으로 작성하거나 저장하려고 하지 마십시오. 대신 ADK가 이를 대신 생성하도록 하십시오.

1. 기록 플러그인을 활성화한 상태로 ADK 웹 서버를 시작합니다.

```shell
adk web -v --extra_plugins=google.adk.cli.plugins.recordings_plugin.RecordingsPlugin /path/to/agents
```

2. 다음으로, 새 터미널 창을 열고 ADK에 `spec.yaml`을 기반으로 기준선 파일을 생성하도록 지시합니다.

```shell
adk conformance create tests/category/test_name
```

이렇게 하면 시나리오가 자동으로 실행되고, 모든 상호 작용이 기록되며, `generated-recordings.yaml` 및 `generated-session.yaml` 파일이 필요한 위치에 정확히 저장됩니다.

이러한 기준선 파일이 확정되면 설정이 완료되며, 디렉터리는 `adk conformance`에 의해 **Replay**(재생) 또는 **Live**(실제) 모드에서 실행될 준비가 됩니다.

#### 작동 방식

* **Replay 모드 (기본값):** 도구가 에이전트를 실행하고 해당 라이브 LLM 요청, 응답 및 도구 호출을 이전에 기록된 상호 작용과 직접 비교하여 예상치 못한 이탈을 감지합니다.
* **Live 모드:** 활성 환경에 대해 평가 기반 검증을 실행합니다 *(참고: 이 모드는 개발 중입니다)*.

### 평가 기준

ADK는 도구 궤적 일치에서 LLM 기반 응답 품질 평가에 이르기까지 에이전트 성능을 평가하기 위한 여러 기본 제공 기준을 제공합니다. 사용 가능한 기준 및 사용 시기에 대한 자세한 목록은 [평가 기준](./criteria.md)을 참고하세요.
...
*   **멀티 턴 사용자 시뮬레이터 품질 평가:** `per_turn_user_simulator_quality_v1`을 사용하여 사용자 시뮬레이터의 품질을 평가합니다.
...
대화형 에이전트를 평가할 때 대화가 예기치 않은 방식으로 진행될 수 있으므로 고정된 사용자 프롬프트 세트를 사용하는 것이 항상 실용적인 것은 아닙니다. 예를 들어 에이전트가 작업을 수행하기 위해 사용자에게 두 개의 값을 제공해야 하는 경우 해당 값을 한 번에 하나씩 또는 한 번에 모두 요청할 수 있습니다. 이 문제를 해결하기 위해 ADK를 사용하면 AI 모델에 의해 동적으로 생성되는 사용자 프롬프트를 사용하여 특정 *대화 시나리오*에서 에이전트의 동작을 테스트할 수 있습니다. 사용자 시뮬레이션을 사용한 평가 설정에 대한 자세한 내용은 [사용자 시뮬레이션](./user-sim.md)을 참고하세요.

## ADK로 평가를 실행하는 방법

개발자는 다음과 같은 방법으로 ADK를 사용하여 에이전트를 평가할 수 있습니다.

- **웹 기반 UI (**`adk web`**):** 웹 기반 인터페이스를 통해 대화형으로 에이전트를 평가합니다.
- **프로그래밍 방식 (**`pytest`**):** `pytest` 및 테스트 파일을 사용하여 평가를 테스트 파이프라인에 통합합니다.
- **명령줄 인터페이스 (**`adk eval`**):** 명령줄에서 직접 기존 평가 세트 파일에 대한 평가를 실행합니다.
- **일치성 테스트** (**`adk conformance`**):** 예상치 못한 이탈이나 회귀를 감지하기 위해 기준선 파일에 대해 자동화된 테스트를 실행합니다.
...
모든 테스트를 한 번에 실행하거나, 특정 테스트만 실행하거나, 요약 보고서를 생성할 수 있습니다.

#### 모든 테스트 실행

특정 폴더 경로를 입력하지 않으면 도구는 작업 공간에서 `tests/` 폴더를 자동으로 찾아 그 안의 모든 테스트를 실행합니다.

```shell
adk conformance test
```

#### 특정 테스트 그룹 또는 개별 케이스 실행

하나 이상의 폴더 경로를 전달하여 실행할 테스트를 좁힐 수 있습니다.
...
`--generate_report` 플래그를 추가하면 깔끔한 테스트 요약 보고서를 생성할 수 있습니다. `--report_dir` 매개변수를 사용하여 저장할 위치를 선택적으로 지정할 수 있습니다.
...
`adk conformance test`는 일치하지 않는 부분이 있으면 실패를 반환하는 명령줄 도구이므로 CI/CD 파이프라인에 유용합니다. 풀 요청(pull request)이 열릴 때마다 자동으로 실행되도록 설정하여 에이전트의 예상 동작이 변경되는 경우 병합(merge)을 차단할 수 있습니다.
