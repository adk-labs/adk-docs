...
대상 테스트 폴더에 `spec.yaml` 파일을 생성합니다. 이 파일은 기준선 기록 및 후속 일치성 실행 동안 에이전트가 실행할 초기 조건, 구성 및 사용자 프롬프트를 개략적으로 설명합니다. 파일이 다음 기본 스키마와 일치하는지 확인하십시오 (예시일 뿐입니다).
