...
이 접근 방식은 도구가 두 가지 유형의 입력을 받는다는 사실에 의존합니다: 모델에 의해 설정되는 인수(arguments)와, 에이전트 개발자에 의해 결정론적으로 설정될 수 있는 [**`도구 컨텍스트(Tool Context)`**](../tools-custom/index.md#tool-context)입니다. 우리는 모델이 예상대로 작동하는지 검증하기 위해 결정론적으로 설정된 정보에 의존할 수 있습니다. *(참고: TypeScript에서는 `Tool Context`가 통합된 `Context` 타입에 해당합니다.)*
...
=== "TypeScript"
    ...
    // 개념적 예시: 도구 컨텍스트를 위한 정책 데이터 설정
    // 실제 ADK 앱에서는 InvocationContext.session.state에 설정되거나
    // 도구 초기화 중에 전달된 후 Context를 통해 검색될 수 있습니다.

    const policy: {[key: string]: any} = {};
    policy['select_only'] = true;
    policy['tables'] = ['mytable1', 'mytable2'];

    // 개념적: 나중에 Context를 통해 도구가 접근할 수 있는 곳에 정책 저장
    // 이 특정 줄은 실제로는 다르게 보일 수 있습니다.
    // 예를 들어, 세션 상태에 저장:
...
도구 실행 중에, [**`도구 컨텍스트(Tool Context)`**](../tools-custom/index.md#tool-context)가 도구에 전달됩니다. *(참고: TypeScript에서는 이것이 통합된 `Context` 타입으로 전달됩니다.)*
...
=== "TypeScript"
    ...
    function query(query: string, context: Context): string | object {
        ...
        // --- 플레이스홀더 정책 강제 ---
        const actualTables = explainQuery(query); // 가상 함수 호출

        const policyTables = new Set(policy['tables'] || []);
        const isSubset = actualTables.every((table) => policyTables.has(table));

        if (!isSubset) {
            const allowed = (policy['tables'] || ['(정의되지 않음)']).join(', ');
            return `오류: 쿼리가 승인되지 않은 테이블을 대상으로 합니다. 허용됨: ${allowed}`;
        }
        ...
        console.log(`검증된 쿼리 실행 (가상): ${query}`);
...
=== "TypeScript"
    ...
    function validateToolParams(
        ...
        console.log(`콜백 트리거됨. 도구: ${tool.name}, 인수: ${JSON.stringify(args)}`);

        const expectedUserId = context.state.get("session_user_id");
        const actualUserIdInArgs = args["user_id_param"];

        if (actualUserIdInArgs !== expectedUserId) {
            console.log("검증 실패: 사용자 ID 불일치!");
            return {"error": "도구 호출 차단됨: 사용자 ID 불일치."};
        }

        console.log("콜백 검증 통과됨.");
        return undefined;
    }
...
* **[Model Armor 플러그인](../integrations/model-armor.md)**: ADK와 함께 제공되며 지정된 에이전트 실행 지점에서 잠재적인 콘텐츠 안전 위반을 확인하기 위해 Model Armor API를 쿼리하는 플러그인입니다. *Gemini 심판* 플러그인과 유사하게, Model Armor가 유해 콘텐츠 일치를 발견하면 사용자에게 미리 정해진 응답을 반환합니다.

* **PII 편집 플러그인(PII Redaction Plugin)**: [도구 실행 전 콜백](/ko/plugins/#tool-callbacks)을 위해 설계된 전문화된 플러그인으로, 개인 식별 정보가 도구에 의해 처리되거나 외부 서비스로 전송되기 전에 이를 편집하기 위해 특별히 만들어졌습니다.
