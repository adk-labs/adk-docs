...
* **PII 편집 플러그인(PII Redaction Plugin)**: [도구 실행 전 콜백](/adk-docs/ko/plugins/#tool-callbacks)을 위해 설계된 전문화된 플러그인으로, 개인 식별 정보가 도구에 의해 처리되거나 외부 서비스로 전송되기 전에 이를 편집하기 위해 특별히 만들어졌습니다.
