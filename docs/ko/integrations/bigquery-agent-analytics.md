...
`BigQueryLoggerConfig`를 사용해 플러그인을 사용자 지정할 수 있습니다. `BigQueryAgentAnalyticsPlugin` 생성자는 `**kwargs`도 받을 수 있으며, 이 값들은 `BigQueryLoggerConfig`로 직접 전달됩니다. 덕분에 별도의 config 객체를 만들지 않고도 `batch_size`나 `enabled` 같은 설정 필드를 전달할 수 있습니다.
...
-   **`retry_config`** (`RetryConfig`, 기본값: `RetryConfig()`): 실패한 BigQuery 쓰기를 재시도하기 위한 설정입니다. 하위 필드는 `max_retries`(기본값 `3`), `initial_delay`(기본값 `1.0`초), `multiplier`(기본값 `2.0`), `max_delay`(기본값 `10.0`초)입니다.
-   **`log_session_metadata`** (`bool`, 기본값: `True`): `True`이면 `session_id`, `app_name`, `user_id`, 그리고 세션 `state` 딕셔너리(예: gchat thread-id, customer_id 같은 커스텀 상태)를 포함한 세션 정보를 `attributes` 컬럼에 기록합니다. `temp:` 또는 `secret:` 접두사가 붙은 상태 키는 자동으로 마스킹됩니다. 자세한 내용은 [내장 마스킹](#built-in-redaction)을 참고하세요.
...
-   **`view_prefix`** (`str`, 기본값: `"v"`): 자동 생성 뷰 이름에 붙는 접두사입니다. 기본값 `"v"`를 사용하면 `v_llm_request` 같은 뷰가 생성됩니다. 동일한 데이터 세트에서 여러 플러그인 인스턴스가 서로 다른 테이블에 기록한다면 뷰 이름 충돌을 막기 위해 테이블마다 다른 접두사를 사용하세요(예: `"v_staging"`은 `v_staging_llm_request`를 생성). 비어 있으면 안 됩니다.
...
플러그인은 수명 주기 관리를 위한 몇 가지 공개 메서드를 제공합니다.

-   **`await plugin.flush()`**: 대기 중인 모든 이벤트를 BigQuery로 플러시합니다. 데이터 손실을 막기 위해 종료 전에 호출하세요.
-   **`await plugin.shutdown(timeout=None)`**: 보류 중인 이벤트를 플러시하고 리소스를 해제하면서 플러그인을 정상 종료합니다. 선택적 `timeout` 인수는 config의 `shutdown_timeout` 값을 덮어씁니다.
-   **`await plugin.create_analytics_views()`**: 이벤트 유형별 분석 뷰를 수동으로 다시 생성합니다. 스키마 업그레이드 이후나 뷰를 새로 고쳐야 할 때 유용합니다.
-   **비동기 컨텍스트 매니저**: 플러그인은 자동 시작/종료를 위해 `async with`도 지원합니다.
...
뷰 이름은 `{view_prefix}_{event_type_lowercase}` 규칙을 따릅니다. 예를 들어 기본 접두사 `"v"`를 사용하면 `LLM_REQUEST`는 `v_llm_request`가 됩니다. 같은 데이터 세트 안에서 여러 플러그인 인스턴스가 서로 다른 테이블에 기록하는 경우 뷰 이름 충돌을 막기 위해 `BigQueryLoggerConfig`의 `view_prefix`를 테이블마다 다르게 지정하세요.

```python
# 같은 데이터 세트에서 서로 다른 접두사를 쓰는 두 플러그인 예시
plugin_prod = BigQueryAgentAnalyticsPlugin(
    project_id=PROJECT_ID, dataset_id=DATASET_ID,
    table_id="agent_events_prod",
    config=BigQueryLoggerConfig(view_prefix="v_prod"),
)
# 생성되는 뷰 예시: v_prod_llm_request, v_prod_tool_completed, ...
...
# 생성되는 뷰 예시: v_staging_llm_request, v_staging_tool_completed, ...
```

스키마 업그레이드 이후처럼 뷰를 다시 만들어야 할 때는 공개 비동기 메서드 `await plugin.create_analytics_views()`를 직접 호출할 수도 있습니다.

모든 뷰에는 다음 **공통 컬럼**이 포함됩니다: `timestamp`, `event_type`, `agent`, `session_id`, `invocation_id`, `user_id`, `trace_id`, `span_id`, `parent_span_id`, `status`, `error_message`, `is_truncated`.

다음 표는 자동 생성되는 15개 뷰와 각 뷰의 이벤트별 컬럼을 보여줍니다.

| 뷰 이름 | 이벤트별 컬럼 |
|:---|:---|
| **`v_user_message_received`** | *(공통 컬럼만 포함)* |
...
| **`v_invocation_starting`** | *(공통 컬럼만 포함)* |
| **`v_invocation_completed`** | *(공통 컬럼만 포함)* |
...
에이전트 내부 상태의 변경(예: 도구가 갱신한 사용자 정의 애플리케이션 상태)을 추적합니다.

!!! note "내장 마스킹"

    `temp:` 또는 `secret:` 접두사가 붙은 상태 키는 기록된 `state_delta`에서 자동으로 `[REDACTED]`로 마스킹됩니다. 자세한 내용은 [내장 마스킹](#built-in-redaction)을 참고하세요.
...
!!! note "HITL 이벤트용 뷰"
    ...
    자동 생성되는 뷰는 세 가지 **요청** 이벤트 유형(`v_hitl_credential_request`, `v_hitl_confirmation_request`, `v_hitl_input_request`)에만 존재합니다. 세 가지 `*_COMPLETED` 이벤트 유형은 기본 테이블에는 기록되지만 전용 뷰는 생성되지 않습니다. 이들은 `agent_events` 테이블에서 `WHERE event_type LIKE 'HITL_%_COMPLETED'` 조건으로 직접 조회하세요.
...
`v_llm_response` 뷰를 사용하는 방법을 권장합니다.
...
또는 기본 테이블에서 JSON을 직접 추출해 조회할 수도 있습니다.
...
`v_tool_completed` 뷰를 사용하는 방법을 권장합니다.
...
플러그인에는 일반적인 비밀 값을 자동으로 보호하는 **내장 마스킹** 기능이 포함되어 있습니다. 더 세밀한 제어가 필요하면 그 위에 사용자 지정 마스킹을 추가로 적용할 수 있습니다.

### 내장 마스킹 {#built-in-redaction}

플러그인은 `content` 또는 `attributes` JSON 어디에 나타나든 다음과 같은 잘 알려진 키 이름(대소문자 무시)의 값을 자동으로 마스킹합니다.

`client_secret`, `access_token`, `refresh_token`, `id_token`, `api_key`, `password`

또한 **`temp:`** 또는 **`secret:`** 접두사가 붙은 모든 상태 키는 기록된 `state_delta`에서 자동으로 `[REDACTED]`로 대체됩니다. 즉, credential service가 캐시한 OAuth 토큰처럼 `secret:` 범위 아래 저장된 ADK 세션 상태는 BigQuery에 영구 저장되지 않습니다.

!!! info "추가 설정 불필요"

    내장 마스킹은 구조화된 attributes와 상태 로깅에 대해 항상 활성화되어 있으며, 중첩된 딕셔너리와 attribute 값 안의 JSON 문자열에도 재귀적으로 적용됩니다. 사용자 지정 `content_formatter`는 원시 콘텐츠에 **먼저** 적용되므로, 자유 형식 페이로드 안에 포함될 수 있는 비밀 값을 추가로 마스킹할 때 사용하세요.
