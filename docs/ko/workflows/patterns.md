...
* **LLM 에이전트:** 대규모 언어 모델로 구동되는 에이전트. ([LLM 에이전트](../agents/llm-agents.md) 참조)
* **워크플로 에이전트:** 하위 에이전트의 실행 흐름을 관리하도록 설계된 특수 에이전트(`SequentialAgent`, `ParallelAgent`, `LoopAgent`). ([워크플로 에이전트](../agents/workflow-agents/index.md) 참조)
* **커스텀 에이전트:** `BaseAgent`를 상속받아 비-LLM 로직을 가진 자체 특수 에이전트. ([커스텀 에이전트](../agents/custom-agents.md) 참조)
...
* **중요성:** 이 계층 구조는 [워크플로 에이전트](#workflow-agents-as-orchestrators)의 범위를 정의하고 LLM 기반 위임의 잠재적 대상을 결정하는 데 영향을 줍니다. `agent.parent_agent`를 사용하여 계층 구조를 탐색하거나 `agent.find_agent(name)`를 사용하여 하위 에이전트를 찾을 수 있습니다.
...
=== "TypeScript"
    ...
    const taskDoer = new TaskExecutorAgent({name: 'TaskExecutor'}); // 커스텀 비-LLM 에이전트

    // 부모 에이전트를 생성하고 자식을 subAgents로 할당
    const coordinator = new LlmAgent({
        name: 'Coordinator',
        model: 'gemini-flash-latest',
        description: '인사와 작업을 조정합니다.',
        subAgents: [ // 여기에 subAgents 할당
            greeter,
            taskDoer
        ],
    });
...
* **[`SequentialAgent`](../agents/workflow-agents/sequential-agents.md):** `sub_agents`를 나열된 순서대로 하나씩 실행합니다.
...
=== "TypeScript"
    ...
    const step1 = new LlmAgent({name: 'Step1_Fetch', outputKey: 'data'}); // 출력을 state['data']에 저장
...
* **[`ParallelAgent`](../agents/workflow-agents/parallel-agents.md):** `sub_agents`를 병렬로 실행합니다. 하위 에이전트의 이벤트는 서로 섞여 발생할 수 있습니다.
...
=== "Go"
  ...
  * **[`LoopAgent`](../agents/workflow-agents/loop-agents.md):** `sub_agents`를 루프 안에서 순차적으로 실행합니다.
...
=== "TypeScript"
      ...
      import { InvocationContext } from '@google/adk/agents/invocation_context';

      class CheckCondition extends BaseAgent { // 상태를 확인하는 커스텀 에이전트
          async *runAsyncImpl(ctx: InvocationContext): AsyncGenerator<Event, void, void> {
              const status = ctx.session.state.get("status") ?? "pending";
              const isDone = (status === "completed");
              yield new Event({author: this.name, actions: new EventActions({escalate: isDone})}); // 완료되면 escalate
          }
      }

      const processStep = new LlmAgent({name: "ProcessingStep"}); // state['status']를 업데이트할 수 있는 에이전트
      ...
      // poller가 실행되면 Checker가 escalate할 때(state['status'] == 'completed')까지 또는 10회 반복될 때까지
      // processStep과 Checker를 반복적으로 실행합니다.
      ```
...
* **편의성:** [`LlmAgent`](../agents/llm-agents.md)의 `output_key` 속성은 에이전트의 최종 응답 텍스트(또는 구조화된 출력)를 지정된 상태 키에 자동으로 저장합니다.
...
#### b) LLM 기반 위임 (에이전트 전환) {#b-llm-driven-delegation-agent-transfer}

[`LlmAgent`](../agents/llm-agents.md)의 이해 능력을 활용하여 계층 구조 내의 다른 적합한 에이전트로 작업을 동적으로 라우팅합니다.
...
=== "Java"
    ...
    LlmAgent coordinator = LlmAgent.builder()
        .name("Coordinator")
        .model("gemini-flash-latest") // 또는 원하는 모델
        .instruction("당신은 어시스턴트입니다. 예약 작업은 Booker에게, 정보 요청은 Info에게 위임하세요.")
        .description("메인 코디네이터.")
...
[`LlmAgent`](../agents/llm-agents.md)가 다른 `BaseAgent` 인스턴스를 호출 가능한 함수나 [도구](../tools/index.md)로 취급할 수 있게 합니다.
...
* **구조:** 중앙 [`LlmAgent`](../agents/llm-agents.md)(코디네이터)가 여러 전문화된 `sub_agents`를 관리합니다.
...
* **구조:** [`SequentialAgent`](../agents/workflow-agents/sequential-agents.md)가 고정된 순서로 실행되는 `sub_agents`를 포함합니다.
...
* **구조:** [`ParallelAgent`](../agents/workflow-agents/parallel-agents.md)가 여러 `sub_agents`를 동시에 실행하고, 종종 (`SequentialAgent` 내의) 후속 에이전트가 결과를 집계합니다.
...
* **구조:** 일반적으로 [`SequentialAgent`](../agents/workflow-agents/sequential-agents.md) 내에 두 개의 에이전트(생성자 및 비평가/검토자)를 포함합니다.
...
* **구조:** 여러 반복에 걸쳐 작업을 수행하는 하나 이상의 에이전트를 포함하는 [`LoopAgent`](../agents/workflow-agents/loop-agents.md)를 사용합니다.
...
### 인간 참여 루프(Human-in-the-Loop) 패턴 { #human-in-the-loop-pattern }
