...
이 가이드는 ADK(Agent Development Kit) for Python을 사용하여 시작하는 방법을 보여줍니다. 시작하기 전에 다음이 설치되어 있는지 확인하세요.
...
!!! note
    ...
    `my_agent/` 폴더가 포함된 **상위 디렉토리**에서 이 명령을 실행하세요. 예를 들어 에이전트가 `agents/my_agent/` 안에 있는 경우 `agents/` 디렉토리에서 `adk web`을 실행하세요.
