...
이 가이드는 ADK(Agent Development Kit) for Go를 사용하여 시작하는 방법을 보여줍니다. 시작하기 전에 다음이 설치되어 있는지 확인하세요.
...
내장된 [Google 검색 도구](/ko/tools/built-in-tools/#google-search)를 사용하는 기본 에이전트 코드를 생성합니다. 프로젝트 디렉토리의 `my_agent/agent.go` 파일에 다음 코드를 추가합니다.
