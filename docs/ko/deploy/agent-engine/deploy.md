...
지원되는 리전 목록을 확인하세요.
`adk deploy agent_engine`의 CLI 옵션은 [ADK CLI Reference](https://adk.dev/api-reference/cli/cli.html#adk-deploy-agent-engine)를 참고하세요.
...
배포된 에이전트 테스트 및 상호작용 방법은 [Agent Engine에서 배포된 에이전트 테스트](/deploy/agent-engine/test/)를 참고하세요.
...
자세한 내용은 [Agent Engine에서 배포된 에이전트 테스트](/deploy/agent-engine/test/)를 참고하세요.
