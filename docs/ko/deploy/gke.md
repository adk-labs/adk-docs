...
### Agent Platform용 Workload Identity 구성

에이전트가 Agent Platform을 사용하는 경우, 클러스터에서 실행되는 워크로드가 Agent Platform API를 호출할 수 있는 권한이 필요합니다. 수동 경로와 달리 `adk deploy gke`는 `default` 네임스페이스의 `default` Kubernetes 서비스 계정을 사용하는 매니페스트를 생성합니다. 에이전트가 Gemini와 같은 모델에 액세스할 수 있도록 Workload Identity를 통해 해당 서비스 계정에 `Agent Platform User`(Agent Platform 사용자) 역할을 부여합니다.

!!! tip "AI Studio를 사용하는 경우 생략 가능"
...
Google Cloud 프로젝트를 사용하면서 이 단계를 건너뛰면, 에이전트의 Pod는 성공적으로 시작되지만 배포를 확인할 때 모델에 대한 요청이 `403 PERMISSION_DENIED` 오류로 실패합니다.
...
이것은 일반적으로 Kubernetes 서비스 계정이 Vertex AI API에 접근하는 데 필요한 권한이 없음을 의미합니다. [Vertex AI를 위한 Kubernetes 서비스 계정 구성](#configure-kubernetes-service-account-for-vertex-ai) 섹션에 설명된 대로 서비스 계정을 생성하고 `Vertex AI 사용자` 역할에 바인딩했는지 확인하세요. `adk deploy gke`로 배포한 경우, [Agent Platform용 Workload Identity 구성](#configure-workload-identity-for-agent-platform) 섹션의 설명에 따라 `default` 서비스 계정을 대신 바인딩하세요. AI Studio를 사용하는 경우, 배포 매니페스트에 `GOOGLE_API_KEY` 환경 변수를 설정하고 유효한지 확인하세요.
