...
이 지침은 배포를 위해 수정할 기존 ADK 프로젝트가 있다고 가정합니다. ADK 프로젝트가 없거나 테스트 프로젝트를 사용하려면, Python [빠른 시작](/ko/get-started/quickstart/) 가이드를 완료하여 [multi_tool_agent](https://github.com/google/tree/main/examples/python/snippets/get-started/multi_tool_agent) 프로젝트를 생성하세요. 다음 지침에서는 `multi_tool_agent` 프로젝트를 예시로 사용합니다.
...
ASP 도구는 배포를 위해 프로젝트에 더 많은 파일을 추가합니다. 아래 절차는 기존 프로젝트 파일을 수정하기 전에 백업합니다. 이 가이드는 [multi_tool_agent](https://github.com/google/tree/main/examples/python/snippets/get-started/multi_tool_agent) 프로젝트를 참조 예시로 사용합니다. 원래 프로젝트의 파일 구조는 다음과 같습니다.
