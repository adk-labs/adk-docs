...
複数の並列ノードへ実行を分岐させるグラフを作成でき、通常は各ノードの出力を集約してさらに処理する必要があります。このタスク実行パターンには2つの段階があります。ワークフローは、複数の並列タスクを開始するときにまずファンアウト（fan out）し、次のステップに進む前にそれらのタスクが完了した時点でそれらのパスを再ジョイン（join）します。

ジョインのステップは、各並列タスクの完了を待ち、それらのノードからの出力のコレクションを次のノードに渡す ***JoinNode*** オブジェクトを使用して実現します。
...
次のコードスニペットは、***START*** から3つの並列タスクを開始し、基本的な ***JoinNode*** オブジェクトを使用して最終タスクを実行する前にそれらの出力をジョインする方法を示します。
