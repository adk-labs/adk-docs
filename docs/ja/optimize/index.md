...
ADKは、adk optimizeコマンドラインを使用して実行できるいくつかのサンプラーとエージェントオプティマイザーを提供しています。利用可能なオプションは以下の通りです。
...
`SimplePromptOptimizer` は、実証的な評価データに基づいてエージェントのルートシステム指示（システムプロンプト）を体系的に改善するように設計された、自動化された反復的なプロンプトチューニングコンポーネントです。複数の候補エージェントの多様なパレートフロンティア（Pareto frontier）を維持する GEPA ベースのオプティマイザーとは異なり、`SimplePromptOptimizer` は、指定された一連の反復（iterations）にわたって単一の主要なプロンプトの洗練のみに完全に焦点を当てた、直接的かつシーケンシャルな最適化ループを実行します。

オプティマイザーは、次の4段階の非同期フィードバックループを自動的に実行します。

1. **実行 (Execute)**: 対象のエージェントは、`Sampler` クラスの実装によって管理される評価タスクの特定のバッチを処理します。
2. **評価 (Evaluate)**: サンプラーは、評価データセットに対してエージェントの出力をスコアリングし、構造化された `SamplingResult` を返します。
3. **批判 (Critique)**: 基底となる最適化用大規模言語モデル（LLM）（デフォルトは Gemini-2.5-flash）が、現在のプロンプトとともに過去の評価スコアを分析し、特定の動作上の弱点や不足している点を特定します。
4. **書き換え (Rewrite)**: 最適化モデルは、発見された弱点に対処するようにカスタマイズされたシステムプロンプトの更新されたバリエーションを生成します。この新しいプロンプトは、次の反復に直接入力されます。

**注意:** 最適化ループは、既存のエージェントインスタンスをインプレースで変更（mutate）しません。完了すると、プロセス中に抽出された最も高いスコアを持つエージェントバリエーションを含む `OptimizerResult` を返します。

### 設定

`SimplePromptOptimizerConfig` インスタンスをオプティマイザーに渡すことで、ループの動作を設定します。

| パラメータ | 型 | デフォルト値 | 説明 |
| :---- | :---- | :---- | :---- |
| `num_iterations` | int | *必須* | 実行する最適化ラウンドの総数。 |
| `batch_size` | int | *必須* | 各反復においてサンプラーが処理する評価サンプルケースの数。 |
