...
=== "TypeScript"
    TypeScriptでは `Event` 型のインターフェースです。

    ```typescript
    import {Content} from '@google/genai';

    /**
     * イベントの概念的な構造 (TypeScript)
     */
    export interface Event extends LlmResponse {
      ...
      /** 応答のコンテンツ。 */
      content?: Content;
      // ... errorCode, errorMessage などの他の LlmResponse フィールド
    }
    ```

=== "Go"
    Goでは、これは`google.golang.org/adk/v2/session.Event`型の構造体です。
...
=== "TypeScript"
    ...
    export async function processEvents(runnerEvents: AsyncIterable<Event>) {
      for await (const event of runnerEvents) {
        console.log(`イベントの送信元: ${event.author}`);

        if (event.content && event.content.parts && event.content.parts.length > 0) {
          if (getFunctionCalls(event).length > 0) {
            console.log('  タイプ: ツール呼び出しリクエスト');
          } else if (getFunctionResponses(event).length > 0) {
            console.log('  タイプ: ツールの結果');
          } else if (event.content.parts[0].text) {
            if (event.partial) {
              console.log('  タイプ: ストリーミングテキストチャンク');
            } else {
              console.log('  タイプ: 完全なテキストメッセージ');
            }
          } else {
            console.log('  タイプ: その他のコンテンツ (例: コードの結果)');
          }
        } else if (
          event.actions &&
          ...
          console.log('  タイプ: 状態/アーティファクトの更新');
        } else {
          console.log('  タイプ: 制御シグナルまたはその他');
        }
      }
    }
    ```
        ...
        export function handleFunctionCalls(event: Event) {
            ...
            if (calls.length > 0) {
                for (const call of calls) {
                    ...
                    const argumentsDict = call.args; // オブジェクト形式です
                    console.log(`  ツール: ${toolName}, 引数: ${JSON.stringify(argumentsDict)}`);
                }
            }
        }
        ```
...
*   **関数応答の詳細:**
    ...
    === "TypeScript"
        ...
        ```typescript
        // 擬似コード: 関数応答を処理する (TypeScript)
        export function handleFunctionResponses(event: Event) {
            const responses = getFunctionResponses(event);
            if (responses.length > 0) {
                for (const response of responses) {
                    const toolName = response.name;
                    const result = response.response; // ツールが返したオブジェクト
                    console.log(`  ツールの結果: ${toolName} -> ${JSON.stringify(result)}`);
                }
            }
        }
        ```
...
*   **状態の変更:** このイベントを生成したステップ中にセッション状態で変更されたキーと値のペアのコレクションを提供します。
    ...
    === "Python"
        ...
        ```
    === "TypeScript"
        `delta = event.actions.stateDelta` (`{key: value}` ペアのオブジェクト)。
        ```typescript
        export function handleStateChanges(event: Event) {
            if (event.actions && Object.keys(event.actions.stateDelta).length > 0) {
                console.log(`  状態の変更: ${JSON.stringify(event.actions.stateDelta)}`);
                // 必要に応じてローカル UI またはアプリケーション状態を更新
            }
        }
        ```
    === "Go"
...
*   **アーティファクトの保存:** 保存されたアーティファクトとその新しいバージョン番号（または関連する`Part`情報）を示すコレクションを提供します。
    ...
    === "TypeScript"
        `artifact_changes = event.actions.artifactDelta` (`{filename: version}` 形式のオブジェクト)。
        ```typescript
        export function handleArtifactChanges(event: Event) {
            if (event.actions && Object.keys(event.actions.artifactDelta).length > 0) {
                console.log(`  保存されたアーティファクト: ${JSON.stringify(event.actions.artifactDelta)}`);
                // UI がアーティファクト一覧を更新する可能性があります
            }
        }
        ```
...
*   **制御フローシグナル:** ブール値のフラグまたは文字列の値を確認します：
    ...
    === "TypeScript"
        *   `event.actions.transferToAgent` (string): 制御を指定エージェントへ渡すべきです。
        *   `event.actions.escalate` (boolean): ループを終了すべきです。
        *   `event.actions.skipSummarization` (boolean): ツール結果を LLM が要約しないようにします。
        ```typescript
        export function handleControlFlow(event: Event) {
            if (event.actions) {
                if (event.actions.transferToAgent) {
                    console.log(`  シグナル: ${event.actions.transferToAgent} に転送`);
                }
                if (event.actions.escalate) {
                    console.log('  シグナル: エスカレート (ループ終了)');
                }
                if (event.actions.skipSummarization) {
                    console.log('  シグナル: ツール結果の要約をスキップ');
                }
            }
        }
        ```
...
*   **使用法:** アプリケーションロジックでイベントストリームをフィルタリングします。
    ...
    === "TypeScript"
        ```typescript
        // 擬似コード: アプリケーションで最終応答を処理する (TypeScript)
        import {
            Event,
            getFunctionResponses,
            isFinalResponse,
            ...
            for await (const event of runnerEvents) {
                ...
                if (isFinalResponse(event)) {
                    ...
                    if (fullResponseText || eventText) {
                        // ストリームの最後、または単発メッセージなら蓄積テキストを使う
                        const finalText = fullResponseText + (event.partial ? '' : eventText);
                        console.log(`ユーザーに表示: ${finalText.trim()}`);
                        fullResponseText = ''; // アキュムレータをリセット
                    } else if (
                        ...
                        console.log(`生のツール結果を表示: ${JSON.stringify(responseData)}`);
                    } else if (event.longRunningToolIds && event.longRunningToolIds.length > 0) {
                        console.log('メッセージを表示: ツールはバックグラウンドで実行中です...');
                    } else {
                        // 該当する場合は他の種類の最終応答を処理
                        console.log('表示: 最終的な非テキスト応答またはシグナル。');
                    }
                }
            }
        }
...
*   **明確な作成者:** カスタムエージェントを構築する際は、履歴内のエージェントアクションの帰属が正しいことを確認してください。フレームワークは通常、LLM/ツールイベントの作成者を正しく処理します。
    ...
    === "TypeScript"
        カスタムエージェントロジックで `Event` を構築する際は作成者を設定してください。例: `createEvent({ author: this.name, ... })`
