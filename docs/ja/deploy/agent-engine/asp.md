...
デプロイ済みエージェントのテストについては、[デプロイ済みエージェントのテスト](/deploy/agent-engine/test/)を参照してください。
...
このガイドは、デプロイ対象の ADK プロジェクトを更新すると仮定しています。ADK プロジェクトがない場合やテスト用プロジェクトを使う場合は、先に [はじめに](/ja/get-started/) のガイドのいずれかを実行してエージェントプロジェクトを作成してください。以下の例では `my_agent` を使用します。
...
デプロイ済みエージェントのテスト方法は
[デプロイ済みエージェントのテスト](/deploy/agent-engine/test/)
を参照してください。
