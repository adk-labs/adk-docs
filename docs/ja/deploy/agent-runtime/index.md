...
Agent Runtime 製品が提供するエージェント サービスの概要。
...
使用するプログラミング言語によっては、次のセクションで説明する追加ライブラリが含まれる場合があります。

=== "Python"

    Python でデプロイする場合、ADK API サーバーや ADK Web ユーザーインターフェースライブラリは*含まれません*。Agent Runtime サービスが ADK API サーバー機能に必要なライブラリを提供します。

=== "Go"

    Go でデプロイする場合、専用の ADK API サーバーが*含まれます*。
