...
### Agent PlatformのWorkload Identity設定

エージェントがAgent Platformを使用する場合、クラスタで実行されるワークロードにはAgent Platform APIを呼び出す権限が必要です。手動パスとは異なり、`adk deploy gke`は`default`ネームスペースの `default` Kubernetesサービスアカウントを使用するマニフェストを生成します。エージェントがGeminiなどのモデルにアクセスできるように、Workload Identityを介してそのサービスアカウントに`Agent Platform User`ロールを付与します。

!!! tip "AI Studioを使用する場合はスキップ可能"

    AI Studioを使用し、APIキーでモデルにアクセスする場合は、この手順をスキップできます。
...
Google Cloudプロジェクトを使用している場合にこの手順をスキップすると、エージェントのPodは正常に起動しますが、デプロイを確認する際にモデルへのリクエストが`403 PERMISSION_DENIED`エラーで失敗します。
...
=== "UIテスト"
    ...
    WebブラウザーでKubernetesのサービスURLにアクセスするだけでエージェントをテストできます。

    ADK dev UIを使用すると、ブラウザーで直接エージェントと対話し、セッションを管理し、実行の詳細を表示できます。
...
これは通常、KubernetesサービスアカウントがVertex AI APIにアクセスするために必要な権限を持っていないことを意味します。[Vertex AI用Kubernetesサービスアカウントの設定](#configure-kubernetes-service-account-for-vertex-ai)セクションで説明されているように、サービスアカウントを作成し、`Vertex AI User`ロールにバインドしたことを確認してください。`adk deploy gke`でデプロイした場合は、[Agent PlatformのWorkload Identity設定](#configure-workload-identity-for-agent-platform)セクションの説明に従って、代わりに`default`サービスアカウントをバインドしてください。AI Studioを使用している場合は、デプロイメントマニフェストに`GOOGLE_API_KEY`環境変数を設定し、それが有効であることを確認してください。
