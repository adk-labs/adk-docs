...
<div class="language-support-tag">
  <span class="lst-supported">ADKでサポート</span><span class="lst-python">Python v0.1.0</span><span class="lst-typescript">TypeScript v0.2.0</span><span class="lst-go">Go v0.1.0</span><span class="lst-java">Java v0.1.0</span><span class="lst-kotlin">Kotlin v0.7.0</span>
</div>
...
Model Context Protocol (MCP) と ADK を使用して構築を開始する際、以下の主要なアーキテクチャの違いを理解しておくことは、より安定し効率的なエージェントを設計する上で役立ちます。
...
*   **セッションの永続性:** `MCPToolset` は、`getstate` および `setstate` メソッドを介したオブジェクトのシリアル化をサポートしています。この機能は、エージェントが Cloud Run や Google Kubernetes Engine (GKE) などの管理された環境にデプロイされたときに、そのコンテキストを維持するのに役立ちます。

!!! note "注記"
    エージェントはライフサイクルイベント中にセッション状態を保持しますが、アクティブな MCP 接続は復元時に自動的に再確立されません。プロセスが復元された後、エージェントは信頼性が高く最新のリンクを確保するために、必要に応じて MCP サーバーへの接続を再初期化します。
...
=== "Kotlin"
    ...
    // ADK エージェントはストリーミング可能な HTTP を通じてリモート MCP サービスに接続します
    // headerProvider は suspend 関数のため、fetchToken() でリクエストごとに新しいトークンを await できます。
    // セッションの再利用が無効化されるため、固定ヘッダーの場合は StreamableHttp(headers = ...) を使用してください。
    val toolset =
