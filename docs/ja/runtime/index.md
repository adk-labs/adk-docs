...
-   :material-web:{ .lg .middle } **Dev UI**
    ...
    `adk web` を使って、エージェントと対話するためのブラウザーベース UI を
    起動します。

    [:octicons-arrow-right-24: Web インターフェースを使用する](web-interface/index.md)
...
-   :material-access-point:{ .lg .middle } **アンビエントエージェント**

    ---

    イベントを処理し、システムを監視し、人間の介入なしに非同期で応答する
    自律エージェントを構築します。

    [:octicons-arrow-right-24: アンビエントエージェントを使用する](ambient-agents.md)
...
- **[Resume Agents](resume.md)**: 以前の状態からエージェント実行を再開する
  方法を学びます。
- **[Cancel Agent Runs](cancel.md)**: AbortSignal (TypeScript) を使用して、
  実行中のエージェント呼び出しを正常にキャンセルします。
