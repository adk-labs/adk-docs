...
WebRTC や電話のブリッジが必要な場合、ADK に用意されています。
[LiveKit ランナー](../integrations/livekit.md) はライブエージェントを LiveKit ルームに接続し、メディアキャプチャ、再生、バージイン（割り込み）、SIP を処理するため、カスタムサーバーを作成する必要はありません。
