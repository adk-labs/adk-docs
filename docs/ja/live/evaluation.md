...
既存の評価ケースはそのまま維持されます。同じ会話シナリオや固定された会話を使用してテキスト実行と音声実行の両方を駆動できるため、既存のスイートを再利用できます。完全なスキーマ、ペルソナ、およびシナリオの作成方法については、[オーディオユーザーシミュレーション](../evaluate/user-sim.md#audio-user-simulation-for-live-agents) を参照してください。
