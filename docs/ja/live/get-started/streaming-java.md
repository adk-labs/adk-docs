# Java でライブストリーミングエージェントを構築する

ADK ストリーミングを使用して、低遅延の双方向音声会話を行う Java エージェントを構築します。

Java と Maven をセットアップし、プロジェクトの依存関係を定義して、`ScienceTeacherAgent` を構築します。まず Dev UI でテキストストリーミングとしてテストし、次にライブオーディオを有効にして実際に会話してみます。
...
*   Java 用の **Maven** ビルドツールが必要ですので、先に進む前に [Maven をインストール](https://maven.apache.org/install.html) してください（Cloud Top や Cloud Shell にはすでにインストールされていますが、ローカルの開発環境にはない場合があります）。
...
**ステップ 1:** ブラウザーで、表示された URL（通常は `http://localhost:8080` または `http://127.0.0.1:8080`）を直接開きます。
...
お好みのブラウザーで、以下にアクセスしてください: [http://127.0.0.1:8080/](http://127.0.0.1:8080/)
...
ビデオを試すには、Web ブラウザーをリロードしてカメラボタンをクリックし、ビデオ入力を有効にして「何が見えますか？」と質問してください。エージェントはビデオ入力で見えている内容を回答します。
...
次に、音声とターン検出を設定するには [設定](../configuration.md) を、ライブエージェントにツールを提供するには [ツール](../tools.md) を参照してください。
