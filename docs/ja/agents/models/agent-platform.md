...
<div class="language-support-tag">
    <span class="lst-supported">ADKでサポート</span><span class="lst-python">Python v0.2.0</span><span class="lst-java">Java v0.1.0</span>
</div>
...
=== "Python"
    ...
    **セットアップ:**
    ...
    1. **エージェント プラットフォーム環境:** 統合されたエージェント プラットフォームのセットアップ (ADC、環境) を確認します。
       Vars、`GOOGLE_GENAI_USE_ENTERPRISE=TRUE`) が完了しました。
...
<div class="language-support-tag">
    <span class="lst-supported">ADKでサポート</span><span class="lst-python">Python v1.34.0</span>
</div>

新しい Claude モデルは、固定されたトークン予算を使用する代わりに、モデルが独自の推論深度を自身で選択する*アダプティブ（adaptive）*拡張思考をサポートしています。ネイティブの Claude パスでは、負の `thinking_budget` がアダプティブ思考にマッピングされます。

推論深度を制御する推奨方法は、`AnthropicGenerateContentConfig` の `effort` フィールドです。
...
agent = LlmAgent(
    model="claude-sonnet-4@20250514",  # Agent Platform 上の Claude モデル ID
    name="claude_reasoning_agent",
    instruction="You are a helpful assistant.",
    generate_content_config=AnthropicGenerateContentConfig(
        effort="high",  # "low"、"medium"、"high"、"xhigh"、"max" のいずれか
    ),
)
```

*   Claude では標準の `thinking_config.thinking_level` はサポートされておらず、無視されます（警告が表示されます）。代わりに `effort` を使用してください。
