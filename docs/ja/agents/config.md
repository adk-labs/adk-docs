...
エージェント設定ファイルを使用して、関数、ツール、サブエージェントなどを組み込むことができる、より複雑なエージェントを構築できます。このページでは、エージェント設定機能を使用してADKワークフローを構築および実行する方法について説明します。エージェント設定形式でサポートされている構文と設定の詳細については、[エージェント設定構文リファレンス](/ja/api-reference/agentconfig/)を参照してください。
...
1.  [インストール](/ja/get-started/installation/#python)の手順に従ってADK Pythonライブラリをインストールします。 *現在Pythonが必要です。* 詳細については、[既知の制限](#known-limitations)を参照してください。
...
!!! Tip
    `adk`コマンドが実行されず、ステップ2でバージョンが一覧表示されない場合は、Python環境がアクティブであることを確認してください。MacおよびLinuxのターミナルで`source .venv/bin/activate`を実行します。他のプラットフォームコマンドについては、[インストール](/ja/get-started/installation/#python)ページを参照してください。
...
ADKの[サンプルリポジトリ](https://github.com/search?q=repo%3Agoogle%2Fadk-python+path%3A%2F%5Econtributing%5C%2Fsamples%5C%2F%2F+.yaml&type=code)または[エージェント設定構文](/ja/api-reference/agentconfig/)リファレンスを参照して、`root_agent.yaml`エージェント設定ファイルのより多くの設定オプションを見つけることができます。
...
ADKエージェント設定で何をどのように構築するかについてのアイデアは、ADKの[adk-samples](https://github.com/search?q=repo:google/adk-python+path:/%5Econtributing%5C/samples%5C//+root_agent.yaml&type=code)リポジトリのYAMLベースのエージェント定義を参照してください。エージェント設定形式でサポートされている構文と設定の詳細については、[エージェント設定構文リファレンス](/ja/api-reference/agentconfig/)を参照してください。
