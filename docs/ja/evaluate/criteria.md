...
`rubric_based_multi_turn_trajectory_quality_v1` | カスタムルーブリックに基づくLLM判定によるマルチターン軌跡品質 | いいえ | はい | はい | はい
...
評価対象の各呼び出しについて、この基準は、エージェントが生成したツール呼び出しのリストを、3つの一致タイプのいずれかを使用して期待されるツール呼び出しのリストと比較します。選択した一致タイプに基づいてツール呼び出しが一致する場合、その呼び出しには1.0のスコアが与えられ、それ以外の場合はスコアは0.0になります。最終的な値は、評価ケースのすべての呼び出しにわたるこれらのスコアの平均です。
...
ルーブリックは、`EvalCase.rubrics` を介してケースごとに添付することもできます。基準レベルのルーブリックとは異なり、これらは `type` によってフィルタリングされます。`type` がこの基準の期待値（`"FINAL_RESPONSE_QUALITY"`）と一致するエントリのみが、有効なルーブリックセットにマージされます。
  ...
  "rubrics": [
    {
      ...
      "rubric_content": {
        "text_property": "エージェントの最終応答は、調べていない製品の価格を捏造しない。"
      },
...
審査員に渡されるマージされたルーブリックリストは、上記の基準レベルのリストと、`EvalCase.rubrics` からのタイプが一致するエントリの和集合です。

#### ルーブリックに関する注意点

- `EvalConfig.criteria["rubric_based_final_response_quality_v1"].rubrics` のルーブリックは **空であってはなりません**。`RubricBasedEvaluator` は初期化時にこれをアサートします。
- `EvalCase.rubrics` のルーブリックは、基準レベルのリストの上に **追加** されるものであり、置き換えるものではありません。審査員に渡される有効なルーブリックセットは、両方の和集合です。
- `EvalCase.rubrics` を介してケースごとに提供されるルーブリックは、`type` によってフィルタリングされます。`type` が `"FINAL_RESPONSE_QUALITY"` のもののみがマージされます。`EvalConfig` 内の基準レベルのルーブリックは、`type` によってフィルタリングされ **ません**。
...
ルーブリックは、`EvalCase.rubrics` を介してケースごとに添付することもできます。基準レベルのルーブリックとは異なり、これらは `type` によってフィルタリングされます。`type` がこの基準の期待値（`"TOOL_USE_QUALITY"`）と一致するエントリのみが、有効なルーブリックセットにマージされます。
  ...
  "rubrics": [
    {
      ...
      "rubric_content": {
        "text_property": "ユーザーは可用性（availability）のみを質問しているため、エージェントは価格ツールを呼び出さない。"
      },
...
- `EvalConfig.criteria["rubric_based_tool_use_quality_v1"].rubrics` のルーブリックは **空であってはなりません**。`RubricBasedEvaluator` は初期化時にこれをアサートします。
- `EvalCase.rubrics` のルーブリックは、基準レベルのリストの上に **追加** されるものであり、置き換えるものではありません。審査員に渡される有効なルーブリックセットは、両方の和集合です。
- `EvalCase.rubrics` を介してケースごとに提供されるルーブリックは、`type` によってフィルタリングされます。`type` が `"TOOL_USE_QUALITY"` のもののみがマージされます。`EvalConfig` 内の基準レベルのルーブリックは、`type` によってフィルタリングされ **ません**。
...
この基準は、LLMを審査員として使用して、ユーザー定義のルーブリックセットに対してマルチターン会話全体のエージェントの動作品質を評価します。

### この基準を使用するタイミング

最終応答だけでなく、マルチターン会話全体にわたるエージェントの *軌跡（trajectory）* の側面を評価する必要がある場合は、この基準を使用します。たとえば、エージェントが文脈の遅延開示後に軌道を修正する方法、有用性と安全性のバランスをとる方法、またはドメイン固有の会話ガイドラインに従う方法などです。Agent Platform Eval SDKに委任し、一般的な軸に沿って軌跡を評価する `multi_turn_trajectory_quality_v1` とは異なり、この基準では、ドメインに合わせてカスタマイズされた yes/no ルーブリックを指定できます。

### 詳細

この基準は、会話全体の対話履歴（ユーザーのターン、エージェントのターン、ツールのやり取り）を蓄積し、提供された各ルーブリックに対して単一のLLMベースの評価を実行します。各ルーブリックについて、審査員はすべてのターンにわたるエージェントの累積的な動作を反映した `yes`（1.0）または `no`（0.0）の判定を下します。評価ケースの最初の N-1 ターンは `NOT_EVALUATED` とマークされ、最後のターンに合計スコアが記録されます。他のLLMベースのメトリックと同様に、審査員モデルは呼び出しごとに複数回サンプリングされ、多数決を使用して集計されます。

### この基準の使用方法

この基準は `RubricsBasedCriterion` を使用します。`EvalConfig` エントリでルーブリックのリストを提供します。個々の `EvalCase` エントリに含まれるルーブリックは、基準レベルのリストの上に追加され、`type` によってフィルタリングされます（以下の注意点を示します）。
      ...
      "rubrics": [
        {
          ...
          "rubric_content": {
            "text_property": "エージェントは、個別化されたアドバイスを提供する前に、個々の要因（年齢、既往症、現在の服用薬）について質問する。"
          }
        },
        {
          "rubric_id": "corrects_after_late_disclosure",
          "rubric_content": {
            "text_property": "ユーザーが会話の後半でリスク関連情報を開示した場合、エージェントは以前のアドバイスをそのままにするのではなく、見直して修正する。"
          }
        }
      ]
    }
...
ルーブリックは、`EvalCase.rubrics` を介してケースごとに添付することもできます。基準レベルのルーブリックとは異なり、これらは `type` によってフィルタリングされます。`type` がこの基準の期待値（`"TRAJECTORY_QUALITY"`）と一致するエントリのみが、有効なルーブリックセットにマージされます。
  ...
  "rubrics": [
    {
      ...
      "rubric_content": {
        "text_property": "このケースで開示された服薬履歴を考慮して、エージェントは推奨を確定する前に薬物相互作用をチェックする。"
      },
...
- `EvalConfig.criteria["rubric_based_multi_turn_trajectory_quality_v1"].rubrics` のルーブリックは **空であってはなりません**。`RubricBasedEvaluator` は初期化時にこれをアサートします。
- `EvalCase.rubrics` のルーブリックは、基準レベルのリストの上に **追加** されるものであり、置き換えるものではありません。審査員に渡される有効なルーブリックセットは、両方の和集合です。
- `EvalCase.rubrics` を介してケースごとに提供されるルーブリックは、`type` によってフィルタリングされます。`type` が `"TRAJECTORY_QUALITY"` のもののみがマージされます。`EvalConfig` 内の基準レベルのルーブリックは、`type` によってフィルタリングされ **ません**。

### 出力と解釈方法

この基準は0.0から1.0の間の全体的なスコアを出力し、1.0はエージェントの軌跡がすべてのルーブリックを満たしたことを示し、0.0はどのルーブリックも満たされなかったことを示します。結果には、詳細なルーブリックごとのスコアも含まれます。値が高いほど優れています。
...
この基準は、ユーザーシミュレーターが会話計画とペルソナに忠実であるかを評価します。

### この基準を使用するタイミング

マルチターン会話におけるユーザーシミュレーターを評価する必要がある場合にこの基準を使用します。`ConversationScenario` で定義された会話計画とペルソナにシミュレーターが従っているかを評価するために設計されています。

### 詳細

この基準は、マルチターン会話においてユーザーシミュレーターが定義済みの `ConversationScenario` に従っているかを判定します。

最初のターンでは、ユーザーシミュレーターの応答が `ConversationScenario` の `starting_prompt` と一致するかを確認します。2ターン目以降は LLM-as-a-judge を使用して、ユーザー応答が `ConversationScenario` の `conversation_plan` と `user_persona` に従っているかを評価します。ペルソナへの準拠は、`UserPersona` に指定された `violation_rubrics` を使って確認します。
...
この基準は、マルチターン会話でエージェントが会話の目標を達成したかを評価します。

### この基準を使用するタイミング

会話の最終結果が、意図した目標をどれだけ達成できたかを測定したい場合に使用します。このメトリックは個々のステップではなく、最終結果に焦点を当てます。

### 詳細

この基準は、マルチターン会話のすべてのターンを考慮して、タスクが成功裏に完了したかどうかを判断します。評価は Vertex AI General AI Eval SDK に委譲されます。

### この基準の使用方法

この基準を使用するには Google Cloud プロジェクトが必要です。Vertex AI SDK が正しく動作するように、エージェントのディレクトリにある `.env` ファイルなどで `GOOGLE_CLOUD_PROJECT` と `GOOGLE_CLOUD_LOCATION` 環境変数を設定してください。

`EvalConfig` の `criteria` 辞書で、この基準のしきい値を指定できます。値は 0.0 から 1.0 の浮動小数点数で、会話が成功と見なされるための最小スコアを表します。
...
この基準は 0.0 から 1.0 のスコアを返します。1.0 に近いほどタスク達成度が高く、0.0 に近いほど目標達成に失敗していることを意味します。

## multi_turn_trajectory_quality_v1

この基準は、会話全体の軌跡品質を評価します。

### この基準を使用するタイミング

目標を達成するために取られた経路が、効率的で効果的かつ論理的かを評価したい場合に使用します。`multi_turn_task_success_v1` が目標達成そのものに焦点を当てるのに対し、この基準はその目標をどのような経路で達成したかを見ます。

### 詳細

この基準は、マルチターン相互作用全体の軌跡品質を評価する参照不要のメトリックです。評価は Vertex AI General AI Eval SDK に委譲されます。

### この基準の使用方法

この基準を使用するには Google Cloud プロジェクトが必要です。Vertex AI SDK が正しく動作するように、エージェントのディレクトリにある `.env` ファイルなどで `GOOGLE_CLOUD_PROJECT` と `GOOGLE_CLOUD_LOCATION` 環境変数を設定してください。

`EvalConfig` の `criteria` 辞書で、この基準のしきい値を指定できます。値は 0.0 から 1.0 の浮動小数点数で、軌跡品質が合格と見なされるための最小スコアを表します。
...
この基準は 0.0 から 1.0 のスコアを返します。1.0 に近いほど軌跡の品質が高く、0.0 に近いほど軌跡が非効率または低品質であることを意味します。

## multi_turn_tool_use_quality_v1

この基準は、マルチターン会話中に行われた関数呼び出しを評価します。

### この基準を使用するタイミング

複数ターンにわたるツールや関数呼び出しの品質、関連性、正確性を評価したい場合に使用します。複雑なマルチステップワークフローで、エージェントが適切なツールをいつどのように選択するかをデバッグするのに役立ちます。

### 詳細

このメトリックは参照軌跡なしで関数呼び出しの挙動を評価します。評価は Vertex AI General AI Eval SDK に委譲されます。

### この基準の使用方法

この基準を使用するには Google Cloud プロジェクトが必要です。Vertex AI SDK が正しく動作するように、エージェントのディレクトリにある `.env` ファイルなどで `GOOGLE_CLOUD_PROJECT` と `GOOGLE_CLOUD_LOCATION` 環境変数を設定してください。

`EvalConfig` の `criteria` 辞書で、この基準のしきい値を指定できます。値は 0.0 から 1.0 の浮動小数点数で、ツール使用品質が合格と見なされるための最小スコアを表します。
...
この基準は 0.0 から 1.0 のスコアを返します。1.0 に近いほど会話全体でのツール使用が優れており、0.0 に近いほどツール使用が不適切だったことを示します。
