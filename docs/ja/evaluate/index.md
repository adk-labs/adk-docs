...
!!! note
    ...
    一部の基準（応答品質、安全性、マルチターン品質など）は、[Vertex Gen AI Evaluation Service API](https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/evaluation)を必要とします。これらを使用するには、`GOOGLE_API_KEY` 環境変数を設定して認証するか、Google Cloud プロジェクトの認証情報（`GOOGLE_CLOUD_PROJECT` および `GOOGLE_CLOUD_LOCATION` と Application Default Credentials の組み合わせ）を使用します。
