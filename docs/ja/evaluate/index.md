...
テストファイルはフォルダーに整理できます。オプションで、フォルダーには評価基準を指定する`test_config.json`ファイルを含めることもできます。
...
### 適合性テスト（conformance testing）による評価

`adk conformance test` コマンドは、AIエージェントが長期にわたって一貫した動作をすることを確認します。現在のエージェントの出力を基準となるグラウンドトゥルース（基準線 / baseline）データと検証することで、コードベースやモデルの更新が意図しないデグレードや先祖返り（regression）を引き起こさないことを保証します。

#### 事前準備と設定

`adk conformance` コマンドで有意義な回帰テストを実行するには、まず最適な「ゴールデン基準線（golden baseline）」を設定する必要があります。適合性テストは、アクティブなエージェントの動作と、以前に記録・検証されたインタラクションを比較することによって動作します。

環境を準備するには、以下のワークフローに従ってください。

##### テストディレクトリ階層の作成

適合性テストは、テストケースを自動的に検出してマッピングするために、厳密なファイルレイアウトに依存しています。以下の構造を使用して、テストディレクトリを初期化します。
...
└── category_name/
    └── test_case_name/
        ├── spec.yaml                  # テストケースの仕様
        ├── generated-recordings.yaml   # 基準線の記録されたインタラクション
        └── generated-session.yaml      # 基準線のセッションデータ
```

!!! note
    エージェントがServer-Sent Events（SSE）を使用する場合、テストフレームワークは同じフォルダー内で追加の `generated-recordings-sse.yaml` および `generated-session-sse.yaml` も検索します。

##### テスト仕様の定義 (spec.yaml)

対象のテストフォルダーに `spec.yaml` ファイルを作成します。このファイルには、基準線の記録時およびその後の適合性実行時にエージェントが実行する初期条件、設定、およびユーザープロンプトの概要を記述します。ファイルが以下の基本スキーマに準拠していることを確認してください（これは一例です）。

```yaml
# Weather Agentのspec.yamlの例
name: "current_weather_check"
description: "エージェントが場所を正確に識別し、天気ツールを呼び出していることを検証します。"
user_prompts:
  - "サンフランシスコの現在の気温は？"
expected_tools:
  - "get_weather_api"
...
バックグラウンドデータ（LLMリクエストやツール呼び出しなど）は複雑であるため、基準線ファイルを手動で記述・保存しようとしないでください。代わりに、ADKに生成を任せます。

1. 記録用プラグインを有効にした状態で、ADK Webサーバーを起動します。

```shell
adk web -v --extra_plugins=google.adk.cli.plugins.recordings_plugin.RecordingsPlugin /path/to/agents
```

2. 新しいターミナルウィンドウを開き、`spec.yaml` に基づいて基準線ファイルを生成するようにADKに指示します。

```shell
adk conformance create tests/category/test_name
```

これにより、シナリオが自動的に実行され、すべてのインタラクションが記録され、`generated-recordings.yaml` および `generated-session.yaml` ファイルが必要な場所に正確に保存されます。

これらの基準線ファイルがロックされると、セットアップは完了です。ディレクトリは `adk conformance` を介して **Replay**（再現）または **Live**（実機）モードでテスト可能になります。

#### 動作の仕組み

* **Replayモード（デフォルト）:** ツールはエージェントを実行し、ライブのLLMリクエスト、レスポンス、ツール呼び出しを、以前に記録されたインタラクションと直接比較して、予期しない乖離を検出します。
* **Liveモード:** アクティブな環境に対して評価ベースの検証を実行します *(注：このモードは開発中です)*。
...
!!! warning "Vertex AI 評価 API の要件"
    ...
    Vertex AI 評価基準（`trajectory_quality_v1`、`pairwise_trajectory_quality_v1`、`safety_v1` など）には、[Vertex Gen AI Evaluation Service API](https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/evaluation) が必要です。これらを使用するには、`GOOGLE_API_KEY` 環境変数を設定して認証するか、Google Cloud プロジェクトの認証情報（`GOOGLE_CLOUD_PROJECT` および `GOOGLE_CLOUD_LOCATION` と Application Default Credentials の組み合わせ）を使用します。
...
*   **マルチターンのユーザーシミュレーター品質を評価する：** `per_turn_user_simulator_quality_v1`を使用して、ユーザーシミュレーターの品質を評価します。
...
すべてのテストを一度に実行することも、特定のテストのみを実行することも、要約レポートを作成することもできます。

#### すべてのテストを実行

特定のフォルダーパスを入力しない場合、ツールはワークスペース内の `tests/` フォルダーを自動的に探し、その中のすべてを実行します。

```shell
adk conformance test
```

#### 特定のテストグループまたは個別ケースを実行

1つ以上のフォルダーパスを渡して、実行するテストを絞り込みます。
...
`--generate_report` フラグを追加すると、クリーンなテスト要約レポートを生成できます。必要に応じて、`--report_dir` パラメータを使用して保存先を指定できます。
...
`adk conformance test` は一致しない項目があると失敗を返すコマンドラインツールであるため、CI/CDパイプラインに最適です。プルリクエストが作成されるたびに自動的に実行されるように設定し、エージェントの期待される動作が変更された場合にマージをブロックできます。
