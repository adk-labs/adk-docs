...
!!! note
    エージェントがServer-Sent Events（SSE）を使用する場合、テストフレームワークは同じフォルダ内で追加の `generated-recordings-sse.yaml` および `generated-session-sse.yaml` も検索します。

##### テスト仕様の定義 (spec.yaml)

対象のテストフォルダに `spec.yaml` ファイルを作成します。このファイルには、基準線の記録時およびその後の適合性実行時にエージェントが実行する初期条件、設定、およびユーザープロンプトの概要を記述します。ファイルが以下の基本スキーマに準拠していることを確認してください（これは一例です）。
...
特定のフォルダパスを入力しない場合、ツールはワークスペース内の `tests/` フォルダを自動的に探し、その中のすべてを実行します。
...
1つ以上のフォルダパスを渡して、実行するテストを絞り込みます。
...
`adk conformance test` は一致しない項目があると失敗を返すコマンドラインツールであるため、CI/CDパイプラインに最適です。プルリクエストが作成されるたびに自動的に実行されるように設定し、エージェントの期待される動作が変更された場合にマージをブロックすることができます。
