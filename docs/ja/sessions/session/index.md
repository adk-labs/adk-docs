...
`Session`は、ユーザーとエージェントの間の単一の会話スレッドを表します。テキストメッセージを毎回最初から書き始めないのと同じように、エージェントも進行中の対話に関するコンテキストを必要とします。ADKの`Session`オブジェクトは、これらの個별の会話スレッドを追跡・管理するために特別に設計されています。
...
=== "TypeScript"
       ...
       ```typescript
        ...
        console.log("--- セッションプロパティの確認 ---");
        console.log(`ID ('id'):                ${exampleSession.id}`);
        console.log(`アプリケーション名 ('appName'): ${exampleSession.appName}`);
        console.log(`ユーザーID ('userId'):         ${exampleSession.userId}`);
        console.log(`状態 ('state'):           ${JSON.stringify(exampleSession.state)}`); // 注：ここでは初期状態のみ表示
        console.log(`イベント ('events'):         ${JSON.stringify(exampleSession.events)}`); // 初期状態は空
        console.log(`最終更新日時 ('lastUpdateTime'): ${exampleSession.lastUpdateTime}`);
        console.log("---------------------------------");
        ...
        console.log("temp_serviceの最終ステータス - ", finalStatus);
       ```
...
=== "Kotlin"
       ...
       ```kotlin
        ...
        println("セッションが正常に作成されました。")

        println("--- セッションプロパティの確認 ---")
        println("ID (`id`):                ${exampleSession.key.id}")
        println("アプリケーション名 (`appName`): ${exampleSession.key.appName}")
        println("ユーザーID (`userId`):         ${exampleSession.key.userId}")
        println("状態 (`state`):           ${exampleSession.state}")
        println("------------------------------------")
...
2.  **`VertexAiSessionService`**
    ...
    *   **要件:**
        ...
        *   この[チュートリアル](https://adk.dev/deploy/agent-runtime/)に従って設定できるReasoning Engineのリソース名/ID。
    ...
    *   **最適な用途:** Google Cloudにデプロイされたスケーラブルな本番アプリケーション、特に他のVertex AI機能と統合する場合。
    ...
    === "Go"
          ...
          modelName := "gemini-flash-latest" // 希望のモデルに置き換えてください
...
`DatabaseSessionService` は、2層のロックアーキテクチャを通じて、同時実行操作中のデータ整合性を保証します。
...
!!! note "注意: ADK Python v1.22.0 でセッションDBスキーマが変更されています"
    ...
    ADK Python v1.22.0 ではセッションデータベースのスキーマが変更されています。
    既存のセッションDBを利用している場合はマイグレーションが必要です。
    詳しくは[セッションDBスキーマの移行](/ja/sessions/session/migrate/)を参照してください。
...
## セッションエラーのトラブルシューティング

実行中、ADK は構成または状態の問題を特定するのに役立つ特定の例外を発生させることがあります。

### `SessionNotFoundError`

ランナーがアクティブなセッションストアに存在しないセッションにアクセスまたは実行しようとしたときに発生します。下位互換性のために `ValueError` を継承しています。

* **一般的な原因:** 無効、期限切れ、または不足している `session_id`。セッションが作成される前に実行しようとした場合。
* **解決方法:** まず `create_session(...)` を介してセッションが存在することを確認するか、`auto_create_session=True` を使用して `Runner` を構築します。
