...
!!! note "`GOOGLE_GENAI_USE_ENTERPRISE` は以前は `GOOGLE_GENAI_USE_VERTEXAI` でした"

    これらの変数名は同等であり、同じ動作をします。`GOOGLE_GENAI_USE_ENTERPRISE` を設定してもエージェントが Agent Platform に接続しない場合は、古いバージョンの ADK を使用している可能性があります。代わりに `GOOGLE_GENAI_USE_VERTEXAI` を使用するか、最新バージョンの ADK にアップデートしてください。
