...
-   **max_output_chars**: (任意) ファイルの読み取りやコマンドの実行から返される最大文字数を制限するために `EnvironmentToolset` で使用するパラメータです。これにより、大容量のファイル内容やコマンド出力がエージェントのコンテキストウィンドウの上限を超えてしまうのを防ぐことができます。
