...
<div class="language-support-tag">
  <span class="lst-supported">ADKでサポート</span><span class="lst-python">Python v0.1.0</span><span class="lst-typescript">TypeScript v0.6.1</span><span class="lst-go">Go v0.1.0</span><span class="lst-java">Java v0.1.0</span><span class="lst-kotlin">Kotlin v0.1.0</span>
</div>
...
=== "TypeScript"
    ...
    // 'imageBytes' に PNG 画像のバイナリデータが入っていると仮定します。
    const imageBytes = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);

    // Node.js 環境では Buffer.from(bytes).toString('base64') を使います。
    const imageArtifact: Part = createPartFromBase64(
      Buffer.from(imageBytes).toString('base64'),
      'image/png',
    );

    console.log(`アーティファクトの MIME タイプ: ${imageArtifact.inlineData?.mimeType}`);
    // 生のバイトへアクセスするには base64 をデコードする必要があります。
    ```
...
=== "TypeScript"
    ...
    console.log(`作成した TypeScript アーティファクトの MIME タイプ: ${pdfArtifact.inlineData?.mimeType}`);
    ```
...
=== "TypeScript"
    ...
    // artifactService が設定されていない場合、コンテキストオブジェクトでアーティファクトメソッドを呼び出すとエラーになります。
    ```
...
アーティファクト対話メソッドは、Python、Go、Java では `CallbackContext`（エージェントおよびモデルのコールバックに渡される）と `ToolContext`（ツールのコールバックに渡される）のインスタンスで直接利用できます。TypeScript では統合された `Context` 上で利用できます。
...
*   **コード例:**
    ...
    === "TypeScript"
        ...
        async function saveGeneratedReport(context: Context, reportBytes: Uint8Array): Promise<void> {
          /** 生成した PDF レポートのバイトをアーティファクトとして保存します。 */
          ...
          try {
            ...
            console.log(`TypeScript アーティファクト '${filename}' をバージョン ${version} として正常に保存しました。`);
          } catch (e: any) {
            console.error(
              `TypeScript アーティファクトの保存エラー: ${e.message}。Runner に ArtifactService が設定されていますか？`,
            );
          }
        }
        ```
        ...
        async function processLatestReport(context: Context): Promise<void> {
          ...
          try {
            ...
            if (reportArtifact?.inlineData) {
              console.log(`最新の TypeScript アーティファクト '${filename}' を正常に読み込みました。`);
              console.log(`MIME タイプ: ${reportArtifact.inlineData.mimeType}`);
              // reportArtifact.inlineData.data（Base64 文字列）を処理
              const pdfData = Buffer.from(reportArtifact.inlineData.data || '', 'base64');
              console.log(`レポートサイズ: ${pdfData.length} バイト。`);
              // ... さらなる処理 ...
            } else {
              console.log(`TypeScript アーティファクト '${filename}' が見つかりません。`);
            }
          } catch (e: any) {
            console.error(
              `TypeScript アーティファクトの読み込みエラー: ${e.message}。ArtifactService が設定されていますか？`,
            );
          }
        }
        ```
...
#### `LoadArtifactsTool` を使用する

回答前に、モデル自身がどの利用可能なアーティファクトをロードするか判断する
必要がある場合は、`LoadArtifactsTool` を追加できます。これは、ユーザーが
アップロード済みファイルや、会話コンテキストには保持せずアーティファクトとして
保存された大きな生成出力についてフォローアップ質問をする場合に便利です。

`LoadArtifactsTool` は、利用可能なアーティファクトをモデルの指示に一覧表示します。
モデルが `load_artifacts` ツールを呼び出すと、ADK は選択されたアーティファクトの
内容をそのリクエストへ一時的に追加し、モデルがファイル内容をコンテキストにして
回答できるようにします。ロードされたアーティファクト内容はセッション履歴に
永続保存されないため、後続ターンで同じアーティファクトが必要な場合、モデルは
もう一度このツールを呼び出す必要があります。
...
=== "Python"
    ...
    このエージェントの `Runner` に `artifact_service` が設定されていることを
    確認してください。設定されていない場合、アーティファクトの一覧表示とロードは
    失敗します。アーティファクトに人間が読みやすい要約が必要な場合は、
...
=== "Go"
    ...
    このエージェントの `runner.Config` に `ArtifactService` が含まれていることを
    確認してください。設定されていない場合、アーティファクトの一覧表示とロードは
    失敗します。
        ...
        async function listUserFiles(context: Context): Promise<string> {
          ...
          try {
            ...
            if (!availableFiles || availableFiles.length === 0) {
              ...
              return `利用可能な TypeScript アーティファクトは次のとおりです:\n${fileListStr}`;
            }
          } catch (e: any) {
            console.error(
              `TypeScript アーティファクトの一覧取得エラー: ${e.message}。ArtifactService が設定されていますか？`,
            );
            return 'エラー: TypeScript アーティファクトを一覧表示できませんでした。';
          }
        }
        ```
...
*   **インスタンス化:**
    ...
    === "TypeScript"
        ...
        try {
          ...
          console.log(`TypeScript GcsArtifactService がバケット ${gcsBucketName} 用に初期化されました`);
          // 環境がこのバケットにアクセスするための認証情報を持っていることを確認してください。
          // 例: Application Default Credentials (ADC)

          // そして Runner に渡します。
          // const runner = new Runner({..., artifactService: gcsService});
        } catch (e: any) {
          // GCS クライアント初期化時の潜在的なエラーを処理します（例: 認証の問題）。
          console.error(`TypeScript GcsArtifactService の初期化エラー: ${e.message}`);
        }
        ```
...
英語原文に追加された Kotlin のアーティファクト例を以下にも掲載します。
